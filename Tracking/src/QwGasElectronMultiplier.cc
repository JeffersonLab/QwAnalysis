...
void QwGasElectronMultiplier::FillVFATWord(Int_t VFAT_index){
  ...
  for (Int_t VFAT_channel=0;VFAT_channel<N_VFAT;VFAT_channel++){
    ...
    if ( VFAT_data & 32768 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel]=0;
    if ( VFAT_data & 16384 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+1]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+1]=0;
    if ( VFAT_data & 8192 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+2]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+2]=0;
    if ( VFAT_data & 4096 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+3]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+3]=0;
    if ( VFAT_data & 2048 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+4]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+4]=0;
    if ( VFAT_data & 1024 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+5]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+5]=0;
    if ( VFAT_data & 512 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+6]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+6]=0;
    if ( VFAT_data & 256 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+7]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+7]=0; 
    if ( VFAT_data & 128 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+8]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+8]=0;
    if ( VFAT_data & 64 )
     fBuffer_VFAT[VFAT_index][16*VFAT_channel+9]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+9]=0;
    if ( VFAT_data & 32 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+10]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+10]=0;
    if ( VFAT_data & 16 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+11]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+11]=0; 
    if ( VFAT_data & 8 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+12]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+12]=0;
    if ( VFAT_data & 4 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+13]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+13]=0;
    if ( VFAT_data & 2 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+14]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+14]=0;
    if ( VFAT_data & 1 )
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+15]=1;
    else
      fBuffer_VFAT[VFAT_index][16*VFAT_channel+15]=0;
  }
...
void QwGasElectronMultiplier::GetBC(Int_t bean_count[], Int_t VFAT_index){//return the VFAT Bean Count
  for(Int_t i=4;i<16;i++){
    bean_count[i-4]=fBuffer_VFAT[VFAT_index][i];
  }
};
void QwGasElectronMultiplier::GetEC(Int_t error_check[], Int_t VFAT_index){//return the VFAT Event Count
  for(Int_t i=20;i<28;i++){
    error_check[i-20]=fBuffer_VFAT[VFAT_index][i];
  }
};
void QwGasElectronMultiplier::GetFlags(Int_t flags[], Int_t VFAT_index){//return the VFAT Flags
  for(Int_t i=28;i<32;i++){
    flags[i-28]=fBuffer_VFAT[VFAT_index][i];
  }
};
void QwGasElectronMultiplier::GetChipId(Int_t chip_id[], Int_t VFAT_index){//return the VFAT ID
  for(Int_t i=36;i<48;i++){
    chip_id[i-36]=fBuffer_VFAT[VFAT_index][i];
  }
};
void QwGasElectronMultiplier::GetChannelData(Int_t channel_data[], Int_t VFAT_index){//return the VFAT Channel Data
  for(Int_t i=48;i<176;i++){
    channel_data[i-48]=fBuffer_VFAT[VFAT_index][i];
    if (channel_data[i-48]){
      //std::cout<<"A Hit "<<VFAT_index<<" "<<i-48+1<<" ROC Bank "<<fCurrentROC_ID<<" "<<fCurrentBank_ID<<std::endl;
      AddHit(VFAT_index,i-48);
    }
  }
};
  
void QwGasElectronMultiplier::AddHit(Int_t VFAT_index,Int_t channel){//Add a new hit to the fGEMHits
  Int_t Strip=0;
  ...
  if (VFAT_index != 2 && VFAT_index != 3){
	...
	Strip=2+4*(128-channel);	
    }
    else if (VFAT_index==1){
      if (channel<=64)
	Strip=258+4*(channel-1);
      else
	Strip=260+4*(128-channel);	
    }
    else if (VFAT_index==4){
      if (channel<=64)
	Strip=257+4*(64-channel);
      else
	Strip=259+4*(channel-65);	
    }
    else if (VFAT_index==5){
      if (channel<=64)
	Strip=3+4*(64-channel);
      else
	Strip=1+4*(channel-65);	
    }
    //Add the radial hit
...
void  QwGasElectronMultiplier::ConstructHistograms(TDirectory *folder, TString &prefix){
  ...
  for (Int_t i=0;i<N_VFAT;i++){
    VFAT[i] = new TH1D(Form("%s_VFAT%d",region.Data(),i ),Form("%s_VFAT%d",region.Data(),i ),VFAT_DATA_SIZE,0,VFAT_DATA_SIZE-1); //create a 1D histogram (double precision)  datastream
    VFAT_BC[i] = new TH1D(Form("%s_VFAT%d_Bean_Count",region.Data(),i),Form("%s_VFAT%d_Bean_Count",region.Data(),i),VFAT_BC_SIZE,0,VFAT_BC_SIZE-1); //create a 1D histogram (double precision)  bean count
    VFAT_EC[i] = new TH1D(Form("%s_VFAT%d_Event_Count",region.Data(),i),Form("%s_VFAT%d_Event_Count",region.Data(),i),VFAT_EC_SIZE,0,VFAT_EC_SIZE-1); //create a 1D histogram (double precision)  error check
    VFAT_Flags[i] = new TH1D(Form("%s_VFAT%d_Flags",region.Data(),i),Form("%s_VFAT%d_Flags",region.Data(),i),VFAT_FLG_SIZE,0,VFAT_FLG_SIZE-1); //create a 1D histogram (double precision)  flags
    VFAT_ChipId[i] = new TH1D(Form("%s_VFAT%d_Chip_ID",region.Data(),i),Form("%s_VFAT%d_Chip_ID",region.Data(),i),VFAT_CID_SIZE,0,VFAT_CID_SIZE-1); //create a 1D histogram (double precision)  chip id
    VFAT_ChannelData[i] = new TH1D(Form("%s_VFAT%d_Channel_Data",region.Data(),i),Form("%s_VFAT%d_Channel_Data",region.Data(),i),VFAT_CHANNEL_DATA_SIZE,0,VFAT_CHANNEL_DATA_SIZE-1); //create a 1D histogram (double precision)  channel data
  }

  //create GEM Histograms
  for (Int_t i=0;i<N_GEM;i++){
    GEM[i]=new TH2D(Form("%s_GEM%d",region.Data(),i ),Form("%s_GEM%d",region.Data(),i ),GEM_TRANSVERSESTRIPS,0,GEM_TRANSVERSESTRIPS-1,GEM_RADIALSTRIPS,0,GEM_RADIALSTRIPS-1 );//2D plot with x is transverse; y is radial
  }
};
void  QwGasElectronMultiplier::FillHistograms(){
  ...
  for (Int_t i=0;i<N_VFAT;i++){    
    for(Int_t j=0;j<VFAT_DATA_SIZE;j++){  
      if (fBuffer_VFAT[i][j]==1)
	VFAT[i]->Fill(j);
      if (fVAFTChannelData[i][j]==1 && j<VFAT_CHANNEL_DATA_SIZE)
	VFAT_ChannelData[i]->Fill(j);
      if(fVAFTChipID[i][j]==1 && j<VFAT_CID_SIZE )
	VFAT_ChipId[i]->Fill(j);
      if(fVAFTBC[i][j]==1 && j<VFAT_BC_SIZE)
	VFAT_BC[i]->Fill(j);
      if(fVAFTEC[i][j]==1 && j<VFAT_EC_SIZE)
	VFAT_EC[i]->Fill(j);
      if(fVAFTFLG[i][j]==1 && j<VFAT_FLG_SIZE)
	VFAT_Flags[i]->Fill(j);    
    }    
  }
...
void  QwGasElectronMultiplier::DeleteHistograms(){
  ...
  for (Int_t i=0;i<N_VFAT;i++){
    ...
    if(VFAT_ChipId[i]!= NULL){
      VFAT_ChipId[i]->Delete();  
      VFAT_ChipId[i] = NULL;
    }
    if(VFAT_ChannelData[i]!= NULL){
      VFAT_ChannelData[i]->Delete();  
