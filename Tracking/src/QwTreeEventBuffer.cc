 ...
 * @param filename Filename of the ROOT file generated by QweakSimG4
	...
	vector <vector <QwDetectorInfo> > & detector_info)
{
  ...
  ReserveVectors();
  ClearVectors();
  // Attach the branches to the vectors
  AttachBranches();
}
...
{
  // If this is the current entry, return without re-reading
  if (entry == fEventNumber) return;
  // Set the current event number
  fEventNumber = entry;

  // Read event
  QwVerbose << "Reading event " << entry << QwLog::endl;
  ...
  if (fRegion1_HasBeenHit && fRegion2_HasBeenHit && fRegion3_HasBeenHit) {
    ...
    QwDebug << "Skipped event with missing hits: " << fEventNumber << QwLog::endl;
  }
...
/**
 * Get the full 'born' event, including all partial track and tree line lists
 * @return Event
 */
QwEvent* QwTreeEventBuffer::GetEvent() const
{
  if (fEventNumber < 0) {
    QwError << "No event has been read yet, use GetEntry(entry)!" << QwLog::endl;
    return 0;
  }
