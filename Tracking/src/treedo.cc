 ...
 /* enum Eorientation orient;*/
  ...
  for (upplow = w_upper; upplow <= w_upper /*w_lower*/; upplow++) {
	...
	for (dir = u_dir; dir <= x_dir /*y_dir*/; dir++) {
          ...
          //cout << upplow << ", " << region << ", " << type << ", " << dir << endl;
...
/* ---- 3rd: create the bit patterns for the hits                     ---- */
	  ...
	  if (region == r3) {
	    ...
	    for (k = 0, rd = rcDETRegion[upplow][region-1][dir], decrease = 0;
	         rd; rd = rd->nextsame, decrease += numWiresr3, k++ ) { /* over the like-pitched planes in a region    */
	      ...
	      H = rd->hitbydet;
	      if (debug) cout << "Set pattern hits" << endl;
	      for (i = 0; i < numWiresr3+1; i++) {
		...
		if (opt.showEventPattern) {
		  ...
		  for (int i = 0; i < (signed int)(1UL<<(opt.levels[upplow][region-1][type]))-1; i++){
		    cout << "<" << channelr3[H->wire-decrease][i] << ">";
		  }
		}
		Hnext = H->nextdet;
		H = Hnext;
...
//	      cout << endl; // somewhere around here a memory leak lurks
	                    // you don't say!!! (wdconinc)

	      TreeSearch.TsSearch(&(rcTreeRegion[upplow][region-1][type][dir]->node),channelr3,
				 hashchannelr3, opt.levels[upplow][region-1][type],numWiresr3,TLAYERS);
	  ...
	  /* Region 2 */
	    ...
	    if (debug) cout << "Setting pattern hits" << endl;
	    for (rd = rcDETRegion[upplow][region-1][dir], tlayers = 0; rd; rd = rd->nextsame,tlayers++) { // over the like-pitched planes in a region
	      ...
	      while (H) {
		TreeSearch.TsSetPoint(rcTreeRegion[upplow][region-1][type][dir]->rWidth,
			rd->WireSpacing, H, H->wire, channel[tlayers], hashchannel[tlayers],
		...
		H = Hnext;
	      ...
	      if(opt.showEventPattern) {
		for(int i = 0; i < (signed int) (1UL << (opt.levels[upplow][region-1][type]))-1; i++) {
		  cout << "<" << channel[tlayers][i] << ">";
		}
		cout << endl;
	      }
            }
	    if (debug) cout << "Search for matching patterns " << endl;
	    //cout << dir << endl;
	    TreeSearch.TsSearch(&(rcTreeRegion[upplow][region-1][type][dir]->node),channel,
				 hashchannel, opt.levels[upplow][region-1][type],0,tlayers);

	    if (debug) cout << "Sort patterns" << endl;
            if (rcTreeRegion[upplow][region-1][type][dir]) {
	      TreeCombine.TlTreeLineSort(trelin,upplow,region,type,dir,
					1UL<<(opt.levels[upplow][region-1][type]-1),tlayers,0);
            }
	    event->treeline[upplow][region-1][type][dir] = trelin;
...
/* ---- TASK 4: Hook up the partial track info to the event info     ---- */
  ...
  if(area){
    cout << iEventNo << " " << area->x << " " << area->mx << ' ' << area->y << ' ' << area->my << endl ;
    ngood++;
  } else {
    cout << iEventNo << endl ;
    nbad++;
