...
Data : November 23rd 2011.

Fit the pass3 data for one IHWP state within the given slug and from the data obtained,
removes the contribution from horizontal and vertical compomenents.
...
const Double_t AT = -4.75;
const Double_t AT_E = 0.117;
...
int main(Int_t argc,Char_t* argv[])
{
 ...
 /*Clear the other arrays*/
  ...
  Canvas1->Print(Form("remove_transverse_from_slugs_%i_%i.png",slug_first,slug_num));
...
void plot_n_fit_data(const Int_t size, Double_t valuesin[],Double_t errorsin[], 
  ...
  // variables to grab transvrese fit info
  ...
  /*******************************************
  *******************************************
   First draw the IN, OUT and IN+OUT values with fits
  **********************************************
  **********************************************/
  ...
  in_pv= fit1->GetParameter(0);
  in_pv_e= fit1->GetParError(0);
  ...
  out_pv= fit2->GetParameter(0);
  out_pv_e= fit2->GetParError(0);
  ...
  /*******************************************
  *******************************************
   Next remove the transverse components from 
   each of the In and OUT data set using the relevant
   fits.
  **********************************************
  **********************************************/
  ...
  //Remove transverse
  for(Int_t i=0;i<size;i++){
    c_val_in[i]= valuesin[i]- AT*( in_pv*cos((TMath::Pi()/180)*(45*(i))) - in_ph*sin((TMath::Pi()/180)*(45*(i)))); 
    c_err_in[i]=sqrt( pow(errorsin[i],2) 
		      + pow(((in_pv*sin((TMath::Pi()/180)*(45*(i))) - in_ph*cos((TMath::Pi()/180)*(45*(i))))*(AT_E*AT_E)),2)
		      + pow((AT*cos((TMath::Pi()/180)*(45*(i)))*in_pv_e),2)
		      + pow((AT*sin((TMath::Pi()/180)*(45*(i)))*in_ph_e),2)
		      + pow(in_p2_e,2)
		      );
    ...
    c_val_out[i]= valuesout[i]-AT*( out_pv*cos((TMath::Pi()/180)*(45*(i))) - out_ph*sin((TMath::Pi()/180)*(45*(i))));
    c_err_out[i]=sqrt( pow(errorsout[i],2) 
		      + pow(((out_pv*sin((TMath::Pi()/180)*(45*(i))) - out_ph*cos((TMath::Pi()/180)*(45*(i))))*(AT_E*AT_E)),2)
		      + pow((AT*cos((TMath::Pi()/180)*(45*(i)))*out_pv_e),2)
		      + pow((AT*sin((TMath::Pi()/180)*(45*(i)))*out_ph_e),2)
		      + pow(out_p2_e,2)
		       );
  ...
  legend1->AddEntry(grp_in_1, "IHWP-IN w/o AT", "p");
  legend1->AddEntry(grp_out_1, "IHWP-OUT w/o AT", "p");
  legend1->AddEntry(grp_sum_1, "(IN+OUT)/2 w/o AT", "p");
 ...
 /*Draw IN values*/
  ...
  // I just use the diff instead of the sum for my ease
  for(Int_t i =0;i<4;i++){
