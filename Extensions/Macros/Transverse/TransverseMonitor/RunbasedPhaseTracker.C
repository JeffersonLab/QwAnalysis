...
/*text file to store individual run based fit results*/
std::ofstream Myfile;    
...
int main(Int_t argc,Char_t* argv[])
{
  ...
  sprintf(textfile,"transverse_phase_in_runs_%i_%i.txt",run_first, run_last); 
  ...
  /*Create a canvas*/
  TString title = Form("Variation of phase offset within runs %i to %i in fit p0*-4.75*Cos(phi+p1)+p2",
		       run_first,run_last);
 ...
 /*set X location and clear the other arrays*/
  ...
  Myfile <<"# ihwp\t\t  run \t\t p0(ppm)+-error(ppm) \t p1(degree)+-p1error(degree) \t chisqure min "<<std::endl;
  ...
  if(ko==0 && ki==0) {std::cout<<"No data!"<<std::endl;
    ...
    Canvas->Update();
    Canvas->Print(Form("transverse_phase_in_runs_%i_%i.png",run_first,run_last));
    Myfile.close();
  }
  std::cout<<"Done! \n";
  db->Close();
	...
	*get_p1 = fit1->GetParameter(2);
	...
	std::cout<<"p1 =" <<*fit_value<< "+-" <<*fit_error << std::endl;
      ...
      Myfile <<ihwp<<"\t\t"<<  run  <<"\t\t "
	     ...
	     <<setw(8)<<setiosflags(ios::fixed)<<setprecision(3)<<fit1->GetParameter(1) <<"\t+- " 
	     <<setw(8)<<setiosflags(ios::fixed)<<setprecision(3)<<fit1->GetParError(1)  <<"\t\t " 
	     <<setw(8)<<setiosflags(ios::fixed)<<setprecision(3)<<min_chisquare<<std::endl;
