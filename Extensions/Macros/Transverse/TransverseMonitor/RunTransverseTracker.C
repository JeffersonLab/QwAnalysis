...
Data : October 12th 2011.

This script is used to track the changes in the transverse polarization X, Y components
within the given run range
Inputs (required) : Start run, end run (run range) 
       (optional) : Turn on the drawing of individual fits by inserting 1.

e.g  ./run_transverse_tracker 42 51  
To print the individual plots for the runs,
     ./run_transevrse_tracker 42 51  1

Output : Plot of % horizontal transerverse (p1*100)  vs run number and plot of % vertical transerverse 
         (p0*100)  vs run number 
         ...
         *.txt file with the p0, p1  values with their errors and the minimum chi-square
         for each slug.
         transverse_in_runs_*_*.png
         transverse_in_runss_*_*.txt

         When the plot run option is selected, those plots will also be saved in 
         transverse_in_run_*_plot.png
...
int main(Int_t argc,Char_t* argv[])
{
  ...
  if(argc == 4){
    ...
    if(atoi(argv[3])== 1) draw_runs = true;
    else {
    ...
    std::cout<<"The correct syntax is ./run_transverse_tracker [firstrun(required)] [lastrun(required)] [draw_runs(optional): 1 ]"<<std::endl;
    exit(1);
  }
  ...
  std::cout<<"this. So it should not be interpreted that all the runs in the run range was included in the given results."<<std::endl;
  ...
  sprintf(textfile,"transverse_in_runs_%i_%i.txt",run_first, run_last); 
  ...
  Myfile <<"# run range "<<run_first<<" to "<<run_last<<std::endl;
 ...
 /*set X location and clear the other arrays*/
  ...
  Myfile <<"# ihwp\t\t  run \t\t"<<setw(8)<<" P_V+-error \t\t P_H+-p1error  \t chisqure min "<<std::endl;
 
  for(Int_t i=run_first;i<run_last+1;i++){
  ...
  if(ko==0 && ki==0) {std::cout<<"No data!"<<std::endl;
    TString text2 = "Unable to find regressed values for the given run range.";
    ...
    TString title = Form("Variation of transverse polarization in runs %i to %i from fit %s",
			 run_first,run_last,fit.Data());
    ...
    Canvas->Modified();
    Canvas->Update();
    Canvas->Print(Form("transverse_in_runs_%i_%i.png",run_first,run_last));
    Canvas->Print(Form("transverse_in_runs_%i_%i.C",run_first,run_last));
    Myfile.close();
...
void fit_octants_data(TString devicelist[],Int_t run, TString ihwp, 
		      Double_t set_p0, Double_t *get_p0, 
		      Double_t set_p1, Double_t *get_p1,
		      Double_t set_p2, Double_t *get_p2,
		      Double_t *fit_value0,Double_t *fit_error0,
  ...
  */
  ...
  if(!empty){
    ...
    /*To draw individual run plots*/
    if(draw_runs){
      TString fit = "-4.74*( P_{V}*cos#phi -  P_{H}*sin#phi ) + constant";
      TString title = Form("Variation of transverse polarization in run %i. Fit %s",
			   run_first,fit.Data());
    ...
    grp1->SetTitle(Form("%s Run %i",ihwp.Data(),run));
	...
	*get_p1 = fit1->GetParameter(2);
    ...
    if(draw_runs){
      Canvas1->Update();
      Canvas1->Print(Form("transverse_in_run_%i_plot.png",run));
    }
    delete fit1;
    delete grp1;
    delete cosfit;
  ...
  if(empty){
    std::cout<<"Found 0 values in detectors. Ignore this run "<<run<<std::endl;
    *fit_value0 = 0.0;
    *fit_error0 = 0.0;   
    *fit_value1 = 0.0;
    *fit_error1 = 0.0;     
