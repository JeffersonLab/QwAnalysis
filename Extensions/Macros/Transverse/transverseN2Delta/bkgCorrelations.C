...
//   tAsymBkg9Out->Draw();
  ...
  Int_t counter13=0;
  ifstream runletAsymMD;
  const Int_t NUM13 = 500;
  Double_t octant13[NUM11],run_number13[NUM11],asym13[NUM11],asymError13[NUM11],asymRMS13[NUM11];
  TString hwp13[NUM11];

  std::vector <Double_t> runMD1in,runMD2in,runMD3in,runMD4in,runMD5in,runMD6in,runMD7in,runMD8in,runMD9in;
  std::vector <Double_t> runMD1out,runMD2out,runMD3out,runMD4out,runMD5out,runMD6out,runMD7out,runMD8out,runMD9out;
  std::vector <Double_t> asymMD1in,asymMD2in,asymMD3in,asymMD4in,asymMD5in,asymMD6in,asymMD7in,asymMD8in,asymMD9in;
  std::vector <Double_t> asymMD1out,asymMD2out,asymMD3out,asymMD4out,asymMD5out,asymMD6out,asymMD7out,asymMD8out,asymMD9out;
  std::vector <Double_t> asymMD1Erin,asymMD2Erin,asymMD3Erin,asymMD4Erin,asymMD5Erin,asymMD6Erin,asymMD7Erin,asymMD8Erin,asymMD9Erin;
  std::vector <Double_t> asymMD1Erout,asymMD2Erout,asymMD3Erout,asymMD4Erout,asymMD5Erout,asymMD6Erout,asymMD7Erout,asymMD8Erout,asymMD9Erout;
  std::vector <Double_t> myMDZero;

  const Int_t detMD = 8;
  Double_t calc_AsymMDInp0[detMD],calc_eAsymMDInp0[detMD],calc_AsymMDInChisquare[detMD],calc_AsymMDInNDF[detMD];
  Double_t calc_AsymMDOutp0[detMD],calc_eAsymMDOutp0[detMD],calc_AsymMDOutChisquare[detMD],calc_AsymMDOutNDF[detMD];


  runletAsymMD.open(Form("dirPlot/%s_%s_%s_%s_MD_PMTavg_regression_%s_%s_%s_asym_%s.txt"
			  ,interaction.Data(),qtor_stem.Data(),polar.Data(),target.Data()
			  ,reg_calc.Data(),reg_set.Data(),outputAvg.Data(),database_stem.Data()));
  

   if (runletAsymMD.is_open()) {
   
    while(1) {
      //       zero13[counter13]=0;
      
      runletAsymMD >> octant13[counter13] >> run_number13[counter13] >> hwp13[counter13]
		    >> asym13[counter13]   >> asymError13[counter13]  >> asymRMS13[counter13] ;
      
      if (!runletAsymMD.good()) break;
      
      if(hwp13[counter13]== "in"){
	
	if(octant13[counter13]==1) {
	  runMD1in.push_back(run_number13[counter13]);
	  asymMD1in.push_back(asym13[counter13]); 
	  asymMD1Erin.push_back(asymError13[counter13]);
	  myMDZero.push_back(0.0);
	}
	if(octant13[counter13]==2) {
	  runMD2in.push_back(run_number13[counter13]);
	  asymMD2in.push_back(asym13[counter13]); 
	  asymMD2Erin.push_back(asymError13[counter13]);
	  myMDZero.push_back(0.0);
	}
	if(octant13[counter13]==3) {
	  runMD3in.push_back(run_number13[counter13]);
	  asymMD3in.push_back(asym13[counter13]); 
	  asymMD3Erin.push_back(asymError13[counter13]);
	  myMDZero.push_back(0.0);
	}
	if(octant13[counter13]==4) {
	  runMD4in.push_back(run_number13[counter13]);
	  asymMD4in.push_back(asym13[counter13]); 
	  asymMD4Erin.push_back(asymError13[counter13]);
	  myMDZero.push_back(0.0);
	}
	if(octant13[counter13]==5) {
	  runMD5in.push_back(run_number13[counter13]);
	  asymMD5in.push_back(asym13[counter13]); 
	  asymMD5Erin.push_back(asymError13[counter13]);
	  myMDZero.push_back(0.0);
	}
	if(octant13[counter13]==6) {
	  runMD6in.push_back(run_number13[counter13]);
	  asymMD6in.push_back(asym13[counter13]); 
	  asymMD6Erin.push_back(asymError13[counter13]);
	  myMDZero.push_back(0.0);
	}
	if(octant13[counter13]==7) {
	  runMD7in.push_back(run_number13[counter13]);
	  asymMD7in.push_back(asym13[counter13]); 
	  asymMD7Erin.push_back(asymError13[counter13]);
	  myMDZero.push_back(0.0);
	}
	if(octant13[counter13]==8) {
	  runMD8in.push_back(run_number13[counter13]);
	  asymMD8in.push_back(asym13[counter13]); 
	  asymMD8Erin.push_back(asymError13[counter13]);
	  myMDZero.push_back(0.0);
	}
	...
	if(octant13[counter13]==1) {
	  runMD1out.push_back(run_number13[counter13]);
	  asymMD1out.push_back(asym13[counter13]); 
	  asymMD1Erout.push_back(asymError13[counter13]);
	}
	if(octant13[counter13]==2) {
	  runMD2out.push_back(run_number13[counter13]);
	  asymMD2out.push_back(asym13[counter13]); 
	  asymMD2Erout.push_back(asymError13[counter13]);
	}
	if(octant13[counter13]==3) {
	  runMD3out.push_back(run_number13[counter13]);
	  asymMD3out.push_back(asym13[counter13]); 
	  asymMD3Erout.push_back(asymError13[counter13]);
	}
	if(octant13[counter13]==4) {
	  runMD4out.push_back(run_number13[counter13]);
	  asymMD4out.push_back(asym13[counter13]); 
	  asymMD4Erout.push_back(asymError13[counter13]);
	}
	if(octant13[counter13]==5) {
	  runMD5out.push_back(run_number13[counter13]);
	  asymMD5out.push_back(asym13[counter13]); 
	  asymMD5Erout.push_back(asymError13[counter13]);
	}
	if(octant13[counter13]==6) {
	  runMD6out.push_back(run_number13[counter13]);
	  asymMD6out.push_back(asym13[counter13]); 
	  asymMD6Erout.push_back(asymError13[counter13]);
	}
	if(octant13[counter13]==7) {
	  runMD7out.push_back(run_number13[counter13]);
	  asymMD7out.push_back(asym13[counter13]); 
	  asymMD7Erout.push_back(asymError13[counter13]);
	}
	if(octant13[counter13]==8) {
	  runMD8out.push_back(run_number13[counter13]);
	  asymMD8out.push_back(asym13[counter13]); 
	  asymMD8Erout.push_back(asymError13[counter13]);
	}
...
//   printf("%s Checking %d\n%s",green,__LINE__,normal);
  ...
  TGraphErrors * asymMD1GraphIN = new TGraphErrors((Int_t)runMD1in.size(),asym1in.data(),asymMD1in.data(),asym1Erin.data(),asymMD1Erin.data());
  asymMD1GraphIN->SetName("asym1GraphIN");
  asymMD1GraphIN->SetMarkerColor(kBlue);
  asymMD1GraphIN->SetLineColor(kBlue);
  asymMD1GraphIN->SetMarkerStyle(20);
  asymMD1GraphIN->SetMarkerSize(markerSize[0]);
  asymMD1GraphIN->SetLineWidth(1);
  asymMD1GraphIN->Draw("AP");
