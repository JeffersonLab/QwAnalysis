...
//   Double_t errout[8] ={0.0};
  ...
  Double_t textTargetX1,textTargetX2,textTargetY1,textTargetY2;
...
//   }
  ...
  if(opt == 1){
    ...
    targ = "HYDROGEN-CELL";
    textTargetX1= 3.1; textTargetY1=0.50; textTargetX2=1.7; textTargetY2=-0.39;
  }
  else if(opt == 2){
    target = "DS-4%-Aluminum";
    targ = "DS 4 % Al";
    textTargetX1= 4.1; textTargetY1=0.40; textTargetX2=2.8; textTargetY2=-0.99;
  }
  else if(opt == 3){
    target = "DS-1.6%-C";
    targ = "DS 1.6 % Carbon";
  ...
  else if(datopt==2){ title1= Form("%s %s, %s",titleSummary.Data(),showFit3.Data(),showFit2.Data());}
  else{
...
//   TPad* pad = (TPad*)(gPad->GetMother());
  ...
  grp_diffH ->Draw("goff");
  grp_diffH ->SetMarkerSize(markerSize[0]);
  grp_diffH ->SetLineWidth(0);
  grp_diffH ->SetMarkerStyle(20);
  grp_diffH ->SetMarkerColor(kOrange+9);
  grp_diffH ->SetLineColor(kOrange+9);
  grp_diffH->Fit("fit_inH","B0");
  TF1* fitH = grp_diffH->GetFunction("fit_inH");
  //  fitH->DrawCopy("same");
  fitH->SetLineColor(kOrange+10);
  fitH->SetLineStyle(2);
  fitH->SetLineWidth(4);
  ...
  grp_diffV ->SetMarkerStyle(21);
  grp_diffV ->SetMarkerColor(kAzure);
  grp_diffV ->SetLineColor(kAzure);
  grp_diffV->Fit("fit_inV","B0");
  TF1* fitV = grp_diffV->GetFunction("fit_inV");
  //fitV->DrawCopy("same");
  fitV->SetLineColor(kAzure+1);
  fitV->SetLineStyle(5);
  fitV->SetLineWidth(4);
...
//   grp->GetYaxis()->SetTitle(Form("MD %s Asymmetry [ppm]",deviceTitle.Data()));
  ...
  statsH->SetTextColor(kOrange+9);
  statsH->SetFillColor(kWhite); 

  statsV->SetTextColor(kAzure);
  statsV->SetFillColor(kWhite); 
  ...
  TLatex* tText_h = new TLatex(textTargetX1,yScale[1]*textTargetY1,Form("Horizontal Transverse"));
  tText_h->SetTextSize(0.045);
  tText_h->SetTextColor(kOrange+9);
  tText_h->Draw();

  TLatex* tText_v = new TLatex(textTargetX2,yScale[1]*textTargetY2,Form("Vertical Transverse"));
  tText_v->SetTextSize(0.045);
  tText_v->SetTextColor(kAzure);
  tText_v->Draw();
  ...
  TString saveSummaryPlot = Form("dirPlot/resultPlot/%s_%s_combined_transverse_%s_MD_%s_regression_%s_%s_slug_summary_plots_%s"
				 ,interaction.Data(),qtor_stem.Data(),polar.Data(),target.Data()
				 ,deviceName.Data(),reg_calc.Data(),reg_set.Data(),database_stem.Data());
