...
// TString quartz_bar_SUM[detNUM]= {"qwk_md1pos","qwk_md2pos","qwk_md3pos","qwk_md4pos",
// 			    "qwk_md5pos","qwk_md6pos","qwk_md7pos","qwk_md8pos"};
// TString deviceTitle = "Pos. PMT"; TString deviceName = "PosPMT";

// TString quartz_bar_SUM[detNUM]= {"qwk_md1neg","qwk_md2neg","qwk_md3neg","qwk_md4neg",
// 			    "qwk_md5neg","qwk_md6neg","qwk_md7neg","qwk_md8neg"};
// TString deviceTitle = "Neg. PMT";  TString deviceName = "NegPMT";

// TString quartz_bar_SUM[5]= {"uslumi1_sum","uslumi3_sum","uslumi5_sum","uslumi7_sum","uslumi_sum"}; 

TString quartz_bar_SUM[detNUM]= {"qwk_uslumi1pos","qwk_uslumi3pos","qwk_uslumi5pos","qwk_uslumi7pos",
				 "qwk_uslumi1neg","qwk_uslumi3neg","qwk_uslumi5neg","qwk_uslumi7neg","uslumi_sum"}; 
TString deviceTitle = "Sum"; TString deviceName = "Sum";

// TString quartz_bar_SUM[detNUM]= {"md1pmtavg","md2pmtavg","md3pmtavg","md4pmtavg",
...
int main(Int_t argc,Char_t* argv[])
{
  ...
  if(ropt == 1){
    good_for = "(lumi_data_view.good_for_id = '3' or lumi_data_view.good_for_id = '1,"+good+"')";
    good_for_sen = "(lumi_slope_view.good_for_id = '3' or lumi_slope_view.good_for_id = '1,"+good+"')";
    interaction = "elastic";
    ...
    good_for_sen = "(lumi_slope_view.good_for_id = '"+good+",18')";
    interaction = "n-to-delta"; 
  }
  else if(ropt == 3){
    good_for = "(lumi_data_view.good_for_id = '1,3,21')";
    good_for_sen = "(lumi_slope_view.good_for_id = '1,3,21')";
  ...
  fileAsym.open(Form("dirPlot/%s_%s_%s_%s_USLumi_%s_regression_%s_%s_%s_asym_%s.txt"
  ...
  fileYield.open(Form("dirPlot/%s_%s_%s_%s_USLumi_%s_regression_off_off_%s_yield_%s.txt"
	  ,interaction.Data(),qtor_stem.Data(),polar.Data(),target.Data()
...
TString get_query(TString detector, TString measurement, TString ihwp){
  ...
  TString datatable     =  "lumi_data_view";
  TString senDatatable  =  "lumi_slope_view";
  TString diffDatatable =  "lumi_slope_view";
...
// Double_t value2222[detNUM],error2222[detNUM];
...
// 	runletid[i] = (Double_t)(asym_stmt->GetDouble(4));     // runlet id
	...
	if(runlet_sulg_opt == 1){
	  if(ldebug){printf("%sUSLumi %s %d %6.0f Asym %s = %g+-%g %g [ppm] %s\n", red, deviceName.Data(), j, run[i], ihwp.Data(), value[i], error[i], rms[i], normal);}
	  fileAsym<<Form("%d %6.0f %s\t %g %g %g", j, run[i], ihwp.Data(), value[i], error[i], rms[i])<<endl;
	}
	if(runlet_sulg_opt == 2){
	  if(ldebug){printf("%sUSLumi %s %d Asym %5.2f %s = %g+-%g %g [ppm] %s\n", red, deviceName.Data(), j, run[i], ihwp.Data(), value[i], error[i], rms[i], normal);}
	  fileAsym<<Form("%d %5.2f %s\t %g %g %g", j, run[i], ihwp.Data(), value[i], error[i], rms[i])<<endl;
	}
	...
	if(runlet_sulg_opt == 1){
	  if(ldebug) {printf("%sUSLumi %s %d %6.0f Yield %s = %g+-%g %g [ppm] %s\n", blue, deviceName.Data(), j, run[i], ihwp.Data(), value[i], error[i], rms[i], normal);}
	  fileYield<<Form("%d %6.0f %s\t %g %g %g", j, run[i], ihwp.Data(), value[i], error[i], rms[i])<<endl;
	}
	
	if(runlet_sulg_opt == 2){
	  if(ldebug) {printf("%sUSLumi %s %d Yield %5.2f %s = %g+-%g %g [ppm] %s\n", blue, deviceName.Data(), j, run[i], ihwp.Data(), value[i], error[i], rms[i], normal);}
	  fileYield<<Form("%d %5.2f %s\t %g %g %g", j, run[i], ihwp.Data(), value[i], error[i], rms[i])<<endl;
	}
