...
//   /* Creating file for writing results */
//   fileAsym.open(Form("dirPlot/%s_%s_%s_%s_MD_%s_regression_%s_%s_asym_%s.txt"
// 	  ,interaction.Data(),qtor_stem.Data(),polar.Data(),target.Data()
// 	  ,deviceName.Data(),reg_calc.Data(),reg_set.Data(),database_stem.Data()));

//   fileYield.open(Form("dirPlot/%s_%s_%s_%s_MD_%s_regression_off_off_yield_%s.txt"
// 	  ,interaction.Data(),qtor_stem.Data(),polar.Data(),target.Data()
// 	  ,deviceName.Data(),database_stem.Data()));

//    get_octant_data(quartz_bar_SUM,"out", value1,  err1, rms1);
//    get_octant_data(quartz_bar_SUM,"in",  value11, err11, rms11);
...
TString get_query(TString detector, TString measurement, TString ihwp){
  ...
  if(runlet_sulg_opt == 1){
    output = "sum( distinct("+datatable+".value/(POWER("
      ...
      +datatable+".error,2))))) , SUM("+datatable+".n ), "+datatable+".slug";
  }
  if(runlet_sulg_opt == 2){
    output = " distinct("+datatable+".value) , "
      +datatable+".error , "+datatable+".error*SQRT("+datatable+".n) , "+datatable+".n , "
      "(run_number + segment_number*0.01) , "+datatable+".runlet_id ";
  }
//   else{
//   if(!runlet_sulg_opt == 1 || !runlet_sulg_opt == 2){
//     printf("%sWrong choise of output \n%s",red,normal); exit(0);
//   }
...
// Double_t value2222[8],error2222[8];
...
//  void get_octant_data(TString *devicelist, TString *ihwp, Double_t *value, Double_t *error, Double_t *rms)
{
  ...
  for(Int_t i=0 ; i<8 ;i++){
	...
	ent[i] =       (Double_t)(asym_stmt->GetDouble(3)); // diff, rms 
	run[i] =      (Double_t)(asym_stmt->GetDouble(4));     // run number
	// 	  runletid[i] = (Double_t)(asym_stmt->GetDouble(4));     // runlet id

	  if(runlet_sulg_opt == 1){
	    if(ldebug){printf("%sMD %s %d %5.0f Asym %s = %g+-%g %g [ppm]  %2.0f %s\n", red, deviceName.Data(), i+1, run[i], ihwp.Data(), value[i], error[i], rms[i], ent[i], normal);}
	    fileAsym<<Form("%d %5.0f %s\t %g %g %g %2.0f", i+1, run[i], ihwp.Data(), value[i], error[i], rms[i], ent[i])<<endl;
	  }
	  if(runlet_sulg_opt == 2){
	    if(ldebug){printf("%sMD %s %d Asym %5.2f %s = %g+-%g %g [ppm] %2.0f %s\n", red, deviceName.Data(), i+1, run[i], ihwp.Data(), value[i], error[i], rms[i], ent[i], normal);}
	    fileAsym<<Form("%d %5.2f %s\t %g %g %g %2.0f", i+1, run[i], ihwp.Data(), value[i], error[i], rms[i], ent[i])<<endl;
	  }
	  
	  //  fileAsym<<Form("%5.2f %s %d\t %g %g %g", run1[i], ihwp.Data(), i+1, value[i], error[i], rms[i])<<endl;
	...
	ent[i] =      (Double_t)(yield_stmt->GetDouble(3));     // run number
	run[i] =      (Double_t)(yield_stmt->GetDouble(4));     // run number

	if(runlet_sulg_opt == 1){
	  if(ldebug) {printf("%sMD %s %d %5.0f Yield %s = %g+-%g %g [ppm] %2.0f %s\n", blue, deviceName.Data(), i+1, run[i], ihwp.Data(), value[i], error[i], rms[i], ent[i], normal);}
	  fileYield<<Form("%d %5.0f %s\t %g %g %g %2.0f", i+1, run[i], ihwp.Data(), value[i], error[i], rms[i], ent[i])<<endl;
	}

	if(runlet_sulg_opt == 2){
	  if(ldebug) {printf("%sMD %s %d Yield %5.2f %s = %g+-%g %g [ppm] %2.0f %s\n", blue, deviceName.Data(), i+1, run[i], ihwp.Data(), value[i], error[i], rms[i], ent[i], normal);}
	  fileYield<<Form("%d %5.2f %s\t %g %g %g %2.0f", i+1, run[i], ihwp.Data(), value[i], error[i], rms[i], ent[i])<<endl;
	}
