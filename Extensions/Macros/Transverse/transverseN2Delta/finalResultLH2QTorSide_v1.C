...
//   dP_sys  = 0.0084; // DocDB 1955.  ELOG Anc 91
  ...
  A_DSal_h      = 6.756; // scalled al asym from 6700 A.
  dADSal_stat_h = 1.186;
  dADSal_sys_h  = 0.000;
  ...
  dAal        = TMath::Sqrt(  pow(dADSal,2) +  pow( 0.50*TMath::Abs(A_DSal - A_USal),2) + pow(dADSalDetAcpt,2) + pow(dAal_nonlin,2) );
...
//   Double_t yPhysScale[2] = {0.0,1.1*dAphys};
//   Double_t yPhysScale[2] = {0.0,42};
...
//   grPhys->SetFillColor(kGray);
  ...
  grPhys->GetYaxis()->SetRangeUser(0.0,yPhysScale);
  TLatex* tPhysText = new TLatex(2.5,yPhysScale*0.85,Form("Combined Horizontal & Vertical"));
...
//   TLatex* tPhysValue = new TLatex(xPhys_n[i]-0.38,yPhys_n[i]+0.10,Form("%0.2f (%0.1f%)",yPhys_n[i],yPhysPercent_n[i]));
  ...
  TString errorPhysPlot = Form("dirPlot/resultPlot/%s_%s_%s_MD_PhysError_Chart_%s"
				 ,interaction.Data(),qtor_stem.Data(),target.Data(),database_stem.Data());
