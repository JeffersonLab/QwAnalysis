...
//  void get_octant_data(TString *devicelist, TString *ihwp, Double_t *value, Double_t *error, Double_t *rms)
{
  ...
  for(Int_t i=0 ; i<8 ;i++){
	...
	// 	  runletid[i] = (Double_t)(asym_stmt->GetDouble(4));     // runlet id
	  ...
	  if(runlet_sulg_opt == 1){
	    if(ldebug){printf("%sMD %s %d %5.0f Asym %s = %g+-%g %g [ppm] %s\n", red, deviceName.Data(), i+1, run[i], ihwp.Data(), value[i], error[i], rms[i], normal);}
	    fileAsym<<Form("%d %5.0f %s\t %g %g %g", i+1, run[i], ihwp.Data(), value[i], error[i], rms[i])<<endl;
	  }
	  if(runlet_sulg_opt == 2){
	...
	if(runlet_sulg_opt == 1){
	  if(ldebug) {printf("%sMD %s %d %5.0f Yield %s = %g+-%g %g [ppm] %s\n", blue, deviceName.Data(), i+1, run[i], ihwp.Data(), value[i], error[i], rms[i], normal);}
	  fileYield<<Form("%d %5.0f %s\t %g %g %g", i+1, run[i], ihwp.Data(), value[i], error[i], rms[i])<<endl;
	}
