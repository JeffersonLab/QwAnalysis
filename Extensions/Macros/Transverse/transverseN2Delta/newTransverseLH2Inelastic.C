...
//   dAmsr_stat_h     = 0.532596;// (std.) reg. scheme, no phi fit
  ...
  A_msr_v          = 4.44129; // (std.) reg. scheme, no phi and C fit
  dAmsr_stat_v     = 0.806615; // (std.) reg. scheme, no phi and C fit

  A_msr_h          = 5.30456; // (std.) reg. scheme, no phi and C fit
  dAmsr_stat_h     = 0.532594; // (std.) reg. scheme, no phi and C fit
...
//   cout<<green<<"A_msr = "<<A_msr<<", DetAcpt ="<<DetAcpt<<", A_msr_DetAcpt = "<<A_msr_DetAcpt<<", dADetAcpt = "<<dADetAcpt<<normal<<endl;
  ...
  P       = 0.88068; // ELOG Anc 91, after update 03/13/2015
  dP_stat = 0.00279; // ELOG Anc 91, after update 03/13/2015
  dP_sys  = 0.00740; // DocDB 1955.  ELOG Anc 91 and 109, after update 03/13/2015
...
//   dADSal_sys    = 0.603014; // same treatment as LH2 data 
  A_DSal        = 8.42622; // std. reg., no phi and C fit, combined H and V transverse. Calculated in a seperate macro "finalResultAl6700A"
  dADSal_stat   = 0.983766; // combined H and V transverse
  dADSal_sys    = 0.603723; // same treatment as LH2 data 
...
//   dAmsr_q2_acceptance   = ((1/pow(dAmsr_stat_v,2))*(dAmsr_q2_acceptance_v) + (1/pow(dAmsr_stat_h,2))*(dAmsr_q2_acceptance_h))/((1/pow(dAmsr_stat_v,2)) + (1/pow(dAmsr_stat_h,2)));
  ...
  //PMT double difference from Kent DocDB 2137, Slide 14.
  dAmsr_PMTDD = 0.500;
  ...
  dAmsr_sys = TMath::Sqrt(  pow(dAmsr_regscheme,2) + pow(dAmsr_reg_time,2) + pow(dAmsr_nonlin,2) + pow(dAmsr_cuts,2) + pow(dAmsr_fit,2) + pow(dAmsr_PMTDD,2)  );
