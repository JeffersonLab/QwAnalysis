...
//   }
  ...
  if(qtor_opt == 1){
    ...
    std::cout<<Form("Unknown Setting!")<<std::endl;
    exit(1);
...
//   line  = Form("*********************************************************************");
  ...
  Double_t dAmsr_trans_h,dAmsr_trans_v,dAmsr_trans;
  Double_t P,dP_stat,dP_sys,dP;
  Double_t Q2,dQ2,Q2_el,dQ2_el,Theta,dTheta,E,dE,W,dW;
  Double_t A_b1,dA_b1,f_b1,df_b1,c_b1;
  Double_t A_b2,dA_b2,f_b2,df_b2,c_b2;
  Double_t A_b3,dA_b3,f_b3,df_b3,c_b3;
  Double_t A_b4,dA_b4,f_b4,df_b4,c_b4,R_b4,dR_b4,A_T_el,dA_T_el,dA_T_el_stat,dA_T_el_sys;
  ...
  if(qtor_opt == 1){
  A_msr_unreg_h    = 7.235;
  dA_msr_unreg_h   = 0.6881;
  }
  else if(qtor_opt == 3){
  A_msr_unreg_h    = 0.9916;
  dA_msr_unreg_h   = 0.4792;
  }
//   A_msr_unreg_h    = 7.235;
//   dA_msr_unreg_h   = 0.6881;
...
//   dADSal_sys    = 0.603565; // same treatment as LH2 data 
  A_DSal        = 8.42787*7.198/5.047;   // scalled al asym from 6700 A.
  dADSal_stat   = TMath::Sqrt( pow((7.198*1.155/5.047),2) + pow((0.707*8.42787/5.047),2) + pow((7.198*8.42787*0.455/pow(5.047,2)),2) ) ;
  dADSal_sys    = 0.0; // 
  }
  else if(qtor_opt == 3){
  A_DSal        = -0.1114; // 5+1 reg., no fit to phi_0
  dADSal_stat   = 1.088;
  dADSal_sys    = 1.140; // same treatment as LH2 data 
  }
...
//   dAmsr_fit_h = 0.018;
  ...
  if(qtor_opt == 1){
  dAmsr_nonlin_h = TMath::Abs(-0.02*A_msr_h);
  }
  else if(qtor_opt == 3){
  dAmsr_nonlin_h = TMath::Abs(-0.02*A_msr_h);
  }
...
//   TLatex * tPhysAsymRDefination = new TLatex(x2,y-0.1,Form("M_{Total} = M_{RC}M_{Det}M_{Bin}M_{Q^{2}}"));
  ...
  Double_t yScale[2],textSize[2],funcYRange[2],yPhysScale[2],yPhysFactor[2],yFactor;

  if(qtor_opt == 1){
  yScale[0] = 0.0;     yScale[1] = 0.98;
  textSize[0] = 0.085; textSize[1] = 0.095;
  funcYRange[0] = 0.0; funcYRange[1] = 200.0;
  yPhysScale[0] = 0.0; yPhysScale[1] = 42.0;
  yPhysFactor[0] = 1.25;   yPhysFactor[1] = 0.4;
  yFactor = 3.0;
  }
  else if(qtor_opt == 3){
  yScale[0] = 0.0;     yScale[1] = 0.80;
  textSize[0] = 0.085; textSize[1] = 0.095;
  funcYRange[0] = 0.0; funcYRange[1] = 90.0;
  yPhysScale[0] = 0.0; yPhysScale[1] = 13.0;
  yPhysFactor[0] = -0.12;   yPhysFactor[1] = -0.12;
  yFactor = 0.85;
  }
...
//   tPhysText->Draw();
  ...
  TLatex* tPhysAxis = new TLatex(xPhys2_n[i],-0.90*yFactor,Form("%s",titlePhys_n[i].Data()));
  tPhysAxis->SetTextSize(0.050);
  tPhysAxis->SetTextAngle(0);
  tPhysAxis->Draw();
  TLatex* tPhysValue = new TLatex(xPhys_n[i]-0.38,yPhys_n[i]+0.80+yPhysFactor[0],Form("%0.2f",yPhys_n[i]));
//   TLatex* tPhysValue = new TLatex(xPhys_n[i]-0.38,yPhys_n[i]+0.10,Form("%0.2f (%0.1f%)",yPhys_n[i],yPhysPercent_n[i]));
  ...
  TLatex* tPhysPercentValue = new TLatex(xPhys_n[i]-0.58,yPhys_n[i]+0.15+yPhysFactor[1],Form("(%0.1f%)",yPhysPercent_n[i]));
  ...
  if(ASYM_DILUTION_PLOT){
    ...
    TString myFuncT = Form("%f*( ((%f/%f) - %f*%f  - %f*%f - %f*x )/(1 - %f - %f - %f - x ) )",M_TotalCorr,A_msr_DetAcpt,P,A_b1,f_b1,A_b3,f_b3,A_b4,f_b1,f_b2,f_b3);
