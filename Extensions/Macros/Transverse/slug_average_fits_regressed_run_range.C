...
//   e.g. use
//   ./slug_average_regressed_run_range run1 run2
//
...
void get_octant_data(Int_t size, TString devicelist[], TString detector_type,TString goodfor,Int_t run1, Int_t run2, Double_t value[], Double_t error[]);
void plot_octant(Int_t size,TString device,Double_t value[],Double_t error[]);
TString get_query(TString detector, TString measurement, TString detector_type, TString goodfor, Int_t run1, Int_t run2);
...
//connect to the data base
  ...
  //plot MD asymmetries
  TString title1 = Form("%s (%s): %i - %i Regressed slug averages of Main detector asymmetries. FIT = p0*cos(phi + p1) + p2",targ.Data(),polar.Data(),run1,run2);
  ...
  pad2->cd();
  get_octant_data(8,quartz_bar_SUM, "MD", goodfor, run1,run2, value1,err1);
  plot_octant(8,"MD BAR SUM",value1,err1);
  gPad->Update();


  Canvas1->Update();
  Canvas1->Print(Form("%i_%i_md_regressed_run_summary_plots.gif",run1,run2));

  // plot US LUMI asymmetries
  TString title2 = Form("%s (%s): %i - %i Regressed slug averages of US Lumi asymmetries. FIT = p0*cos(phi + p1) + p2",targ.Data(),polar.Data(),run1,run2);
  ...
  pad22->cd();
  get_octant_data(4,us_lumi,"LUMI", goodfor,run1,run2,value2,err2);
  plot_octant(4,"US LUMI",value2,err2);
  gPad->Update();

  Canvas2-> Update();
  Canvas2->Print(Form("%i_%i_uslumi_regressed_run_summary_plots.gif",run1,run2));

  // plot DS LUMI asymmetries
  TString title3 = Form("%s (%s): %i - %i Regressed slug averages of DS Lumi asymmetries. FIT = p0*cos(phi + p1) + p2",targ.Data(),polar.Data(),run1,run2);
  ...
  pad222->cd();
  get_octant_data(5,DS_lumi,"LUMI", goodfor,run1,run2,value3,err3);
  plot_octant(5,"DS_LUMI",value3,err3);
  gPad->Update();

  Canvas3-> Update();
  Canvas3->Print(Form("%i_%i_dslumi_regressed_run_summary_plots.gif",run1,run2));
...
// To get rid of runs that have large charge asymmetries
  ...
  TString run_cut = Form("%s.run_number > %i AND %s.run_number < %i+1 AND "
			 ,datatable.Data(),run1,datatable.Data(),run2+1); 
  ...
  TString query =" SELECT " + output
    ...
    + " run.run_number = "+datatable+".run_number AND run.run_type != 'track_noR1' AND "
...
void get_octant_data(Int_t size, TString devicelist[], TString detector_type,TString goodfor,Int_t run1, Int_t run2, Double_t value[], Double_t error[])
{
  ...
  for(Int_t i=0 ; i<size ;i++){
    ...
    TString query = get_query(Form("%s",devicelist[i].Data()),"a",detector_type,goodfor,run1,run2);
