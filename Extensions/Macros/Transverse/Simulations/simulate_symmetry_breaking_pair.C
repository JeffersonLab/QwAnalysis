...
int main(Int_t argc,Char_t* argv[]){
 ...
 TH1D* para_p2 = new TH1D("p2","symmetry breaking term",1000, -0.2,0.2);
 ...
 for(Int_t k=0;k<iterate;k++){
   ...
   for(Int_t i=1;i<9;i++){   
     ...
     else if(i==(bar+4)) angle =(TMath::Pi()/180)*(45*(i-1)+ offset);   
     else
 ...
 para_p0->Draw();
 para_p0->Fit("gaus","Q");
 para_p0->SetTitle(" Phase offset");
 para_p0->GetXaxis()->SetTitle("degrees");
 para_p0->GetYaxis()->SetTitle("No.of Measurements");
 ...
 c1->SaveAs(Form("sim_symmetry_breaking_%1.0f_P_V_%1.0f_P_H_%1.0f_with_md%1.0f_and_md%1.0f_%1.1fdeg.png"
		 ,run_time,PV,PH,bar,bar+4,offset));
