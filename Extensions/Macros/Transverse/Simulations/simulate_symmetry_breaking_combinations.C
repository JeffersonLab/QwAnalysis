...
This macro simulates the transverse symmetry breaking effect on the main detector array when opposite pairs
are shifted in phi

<time_in_hours> Length of each measurement in hours.
 
/sim_symmetry_breaking_pair <time> <bar> <offset in degrees>
...
//   }
 ...
 TH1D* para_p1 = new TH1D("p1","#phi",1000, -5,5);
 ...
 Myfile<<"set1 \t set2  \t set3 \t set4 \t sym breaking (ppm)\t error\t phase offset (deg)\t error\t"<<std::endl;

 Int_t value[4]={0,-1,2,-3};
 Int_t &set1=value[0];
 Int_t &set2=value[1];
 Int_t &set3=value[2];
 Int_t &set4=value[3];
 ...
 Double_t breaking=0;
  ...
  do {
	...
	if((i==1) or (i==5))  angle =(TMath::Pi()/180)*(45*(i-1)+ set1);
	if((i==2) or (i==6))  angle =(TMath::Pi()/180)*(45*(i-1)+ set2);
	if((i==3) or (i==7))  angle =(TMath::Pi()/180)*(45*(i-1)+ set3);
	if((i==4) or (i==8))  angle =(TMath::Pi()/180)*(45*(i-1)+ set4);
    ...
    para_p1->Fit("gaus","Q");
    breaking = (para_p1->GetFunction("gaus"))->GetParameter(1);
      
 
    para_p2->Fit("gaus","Q");
    phaseoff = (para_p2->GetFunction("gaus"))->GetParameter(1);

    Myfile<<set1<<" \t"<<set2<<" \t"<<set3<<" \t"<<set4<<" \t"
	  <<breaking<<" \t" <<(para_p1->GetFunction("gaus"))->GetParError(1)<<" \t"
	  <<phaseoff <<" \t"<<(para_p2->GetFunction("gaus"))->GetParError(1)<<" \t"<<std::endl;  
    std::cout<<set1<<" \t"<<set2<<" \t"<<set3<<" \t"<<set4<<" \t"
	     <<breaking<<" \t"<<phaseoff<<std::endl;      


  } while(TMath::Permute(4,value));
