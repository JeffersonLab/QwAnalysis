...
//    nominal center of the 4% DS Al target, for both packages.
//   
...
void projections(int run=15121, int event_start=0,int event_end=-1,string prefix="", string suffix=""){
   ...
   // only looks at first hit in MD's multihit TDCs - not ideal logic
	...
	x = pt->fOffsetX+z_al*pt->fSlopeX;
	y = pt->fOffsetY+z_al*pt->fSlopeY;
   ...
   // test if the MD is hit - using TDC hits
   // only looks at first hit in MD's multihit TDCs - not ideal logic
      	if(mdm_value_2 <-210 || mdm_value_2 > -150 || mdp_value_2 < -210 || mdp_value_2 > -150)  continue;  // jump out of this event in for loop if not a prompt MD hi
    ...
    *t6= TLine(px[5],py[5],px[3],py[3]);
    ...
    TCanvas* lh2=new TCanvas("c3","4% DS Al Target Projection, Package 1",1000,600);
    TPad* spad4=new TPad("spad4","spad4",.61,.51,.99,.99);
    ...
    p1_projection_lh2->SetTitle(Form("run %d: Package 1:  projection to lH2 target center, Z=%f",run,z_al));
    ...
    *t6= TLine(px[5],py[5],px[3],py[3]);
    ...
    // Center (in z) of lH2 target
    TCanvas* lh2_p2=new TCanvas("c6","4% DS Al Target Projection, Package 2",1000,600);
    TPad* spad4=new TPad("spad4","spad4",.61,.51,.99,.99);
    ...
    p2_projection_lh2->SetTitle(Form("run %d: Package 2:  projection to lH2 target center, Z=%f",run,pkg,z_al));
