...
//
//  based  on  r3_hitprofile_tracks.C
//   
//   Makes Region 3 partial track-based Hit Map at front face of MD
//
...
void project_root(int package=1,int md_number=1,int run_number=6327,int max_events=-1,TString file_suffix="Qweak_",string command="MD_")
{
  ...
  TH1D* yhits = new TH1D("yhits", Form("Y profile on bar, Run %d, Oct. %d", run_number, md_number),220, -110, 110);
  TH1D* xhits = new TH1D("xhits", Form("X profile on bar, Run %d, Oct. %d", run_number, md_number), 60, 320, 350);
  TH1D* yangle = new TH1D("yangle", Form("Y angles on bar, Run %d, Oct. %d", run_number, md_number), 200, -0.2, 0.2);
  TH1D* xangle = new TH1D("xangle", Form("X angles on bar, Run %d, Oct. %d", run_number, md_number)
, 40, 0.32, 0.52);

  TProfile* yangle_v_y;
  yangle_v_y = new TProfile("yangle_v_y", Form("Y angle vs Y Run %d  Oct. %d", run_number, md_number), 220, -110, 110);
  ...
  else if (type=="MD")   dz = md_zpos[md_number] - MD_zoffset;
 ...
 angle  (radians)");
  ...
  yangle_v_y->GetYaxis()->SetTitle("Y angle  (rad)");
  gStyle->SetStatX(0.50); 
  gStyle->SetStatW(0.35); 
  gStyle->SetTitleX(0.2); 
  yangle_v_y->Draw(); 
  gStyle->SetOptFit(1111);
  TF1* fit1 = new TF1("FIT1", "pol1", -100, 100);
  yangle_v_y->Fit(fit1, "R");
  yangle_v_y->SaveAs(outputPrefix+"yangle_v_y.C"); 
  c_yangle_v_y->SaveAs(outputPrefix+"yangle_v_y.pdf"); 
  gStyle->Reset();
