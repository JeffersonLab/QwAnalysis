...
std::string INDEXTOBRANCH[NUMBRANCHES] = {"Treelines","Partial_Tracks","Tracks"};
  ...
  for (int i = 0; i <h_SLOPE_Treelines.size(); i++)
  {
    ...
    for(int j=0; j <h_SLOPE_Treelines[i].size(); j++)
    {
      ...
      ***********************************/
      ...
      {
        h_SLOPE_Treelines[i][j][k] = new TH1D (Form("h_SLOPE_Treelines[%d][%d][%d]",i,j,k),Form("Slope distribution "
          "for treelines in package %d, %s VDC %s plane",i,INDEXTOVDC[j].c_str(),INDEXTOPLANE[k].c_str()),
        ...
        h_CHI2_Treelines[i][j][k] = new TH1D (Form("h_CHI2_Treelines[%d][%d][%d]",i,j,k),Form("Chi squared distribution "
          "for treelines in package %d, %s VDC %s plane",i,INDEXTOVDC[j].c_str(),INDEXTOPLANE[k].c_str()),
        ...
        //parital tracks
        h_SLOPE_Partial_Tracks[i][j][k] = new TH1D (Form("h_SLOPE_Partial_Tracks[%d][%d][%d]",i,j,k),
          Form("Slope distribution for treelines in the partial tracks in package %d, %s VDC %s plane",i,
          INDEXTOVDC[j].c_str(),INDEXTOPLANE[k].c_str()), SLOPE_BIN,SLOPE_MIN,SLOPE_MAX);
        h_SLOPE_Partial_Tracks[i][j][k]->GetYaxis()->SetTitle("Frequency");
        h_SLOPE_Partial_Tracks[i][j][k]->GetXaxis()->SetTitle("Slope");

       h_CHI2_Partial_Tracks[i][j][k] = new TH1D (Form("h_CHI2_Partial_Tracks[%d][%d][%d]",i,j,k),
          Form("Chi squared distribution for treelines in the partial tracks for package %d, %s VDC %s plane",
          i,INDEXTOVDC[j].c_str(),INDEXTOPLANE[k].c_str()), CHI2_BIN,CHI2_MIN,CHI2_MAX);
       h_CHI2_Partial_Tracks[i][j][k]->GetYaxis()->SetTitle("Frequency");
       h_CHI2_Partial_Tracks[i][j][k]->GetXaxis()->SetTitle("Chi squared");

       //tracks
        h_SLOPE_Tracks[i][j][k] = new TH1D (Form("h_SLOPE_Tracks[%d][%d][%d]",i,j,k),
          Form("Slope distribution for treelines in the tracks in package %d, %s VDC %s plane",i,
          INDEXTOVDC[j].c_str(),INDEXTOPLANE[k].c_str()), SLOPE_BIN,SLOPE_MIN,SLOPE_MAX);
        h_SLOPE_Tracks[i][j][k]->GetYaxis()->SetTitle("Frequency");
        h_SLOPE_Tracks[i][j][k]->GetXaxis()->SetTitle("Slope");

        h_CHI2_Tracks[i][j][k] = new TH1D (Form("h_CHI2_Tracks[%d][%d][%d]",i,j,k),
          Form("Chi squared distribution for treelines in the tracks for package %d, %s VDC %s plane",
          i,INDEXTOVDC[j].c_str(),INDEXTOPLANE[k].c_str()), CHI2_BIN,CHI2_MIN,CHI2_MAX);
...
//                h_CHI2_Treelines[0][0][3]->Fill(treeline->fChi);
                ...
                {
                  h_SLOPE_Treelines[treeline->GetPackage()][0][4]->Fill(treeline->GetPackage() == 1 ?
                    - treeline->fSlope : treeline->fSlope);
                  h_CHI2_Treelines[treeline->GetPackage()][0][4]->Fill(treeline->fChi);
          ...
          case 3:
            switch(treeline->GetPlane())
            {
              case 0:
                if(treeline->GetDirection() == 3) // this is the UU plane
                {
                  h_SLOPE_Partial_Tracks[treeline->GetPackage()][0][3]->Fill(treeline->GetPackage() == 1 ?
                    -treeline->fSlope : treeline->fSlope);
                  h_CHI2_Partial_Tracks[treeline->GetPackage()][0][3]->Fill(treeline->fChi);
...
//                 h_CHI2_Partial_Tracks[0][0][3]->Fill(treeline->fChi);
                  ...
                  {
                    h_SLOPE_Partial_Tracks[treeline->GetPackage()][0][4]->Fill(treeline->GetPackage() == 1 ?
                     -treeline->fSlope : treeline->fSlope);
                    h_CHI2_Partial_Tracks[treeline->GetPackage()][0][4]->Fill(treeline->fChi);
...
/*************
              case 1:
                //for package X, front VDC, V plane
               h_SLOPE_Partial_Tracks[treeline->GetPackage()][1][2]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
               h_CHI2_Partial_Tracks[treeline->GetPackage()][1][2]->Fill(treeline->fChi);

                //for both packages X, front VDC, both planes
                h_SLOPE_Partial_Tracks[treeline->GetPackage()][1][0]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
               h_CHI2_Partial_Tracks[treeline->GetPackage()][1][0]->Fill(treeline->fChi);
...
//               h_CHI2_Partial_Tracks[0][1][2]->Fill(treeline->fChi);
              ...
              case 2:
                //for package X, front VDC, U plane
                h_SLOPE_Partial_Tracks[treeline->GetPackage()][1][1]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
               h_CHI2_Partial_Tracks[treeline->GetPackage()][1][1]->Fill(treeline->fChi);
...
//               h_CHI2_Partial_Tracks[0][1][1]->Fill(treeline->fChi);
              ...
              case 3:
                //for package X, Back VDC, V plane
                h_SLOPE_Partial_Tracks[treeline->GetPackage()][2][2]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
               h_CHI2_Partial_Tracks[treeline->GetPackage()][2][2]->Fill(treeline->fChi);

                //for both packages X, back VDC, both planes
                h_SLOPE_Partial_Tracks[treeline->GetPackage()][2][0]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
               h_CHI2_Partial_Tracks[treeline->GetPackage()][2][0]->Fill(treeline->fChi);
...
//               h_CHI2_Partial_Tracks[0][2][2]->Fill(treeline->fChi);
              ...
              case 4:
                //for package X, front VDC, U plane
                h_SLOPE_Partial_Tracks[treeline->GetPackage()][2][1]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
               h_CHI2_Partial_Tracks[treeline->GetPackage()][2][1]->Fill(treeline->fChi);
            ...
            case 3:
              switch(treeline->GetPlane())
              {
                case 0:
                  if(treeline->GetDirection() == 3) // this is the UU plane
                  {
                    h_SLOPE_Tracks[treeline->GetPackage()][0][3]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
                    h_CHI2_Tracks[treeline->GetPackage()][0][3]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
...
//                  -treeline->fSlope : treeline->fSlope);
                    ...
                    {
                      h_SLOPE_Tracks[treeline->GetPackage()][0][4]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
                      h_CHI2_Tracks[treeline->GetPackage()][0][4]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
...
/*****************
                case 1:
                  //for package X, front VDC, V plane
                  h_SLOPE_Tracks[treeline->GetPackage()][1][2]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
                  h_CHI2_Tracks[treeline->GetPackage()][1][2]->Fill(treeline->fChi);

                  //for both packages X, front VDC, both planes
                  h_SLOPE_Tracks[treeline->GetPackage()][1][0]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
                  h_CHI2_Tracks[treeline->GetPackage()][1][0]->Fill(treeline->fChi);
...
//                  h_CHI2_Tracks[0][1][2]->Fill(treeline->fChi);
                ...
                case 2:
                  //for package X, front VDC, U plane
                  h_SLOPE_Tracks[treeline->GetPackage()][1][1]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
                  h_CHI2_Tracks[treeline->GetPackage()][1][1]->Fill(treeline->fChi);
...
//                  h_CHI2_Tracks[0][1][1]->Fill(treeline->fChi);
                ...
                case 3:
                  //for package X, Back VDC, V plane
                  h_SLOPE_Tracks[treeline->GetPackage()][2][2]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
                  h_CHI2_Tracks[treeline->GetPackage()][2][2]->Fill(treeline->fChi);

                  //for both packages X, back VDC, both planes
                  h_SLOPE_Tracks[treeline->GetPackage()][2][0]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
                  h_CHI2_Tracks[treeline->GetPackage()][2][0]->Fill(treeline->fChi);
...
//                  h_CHI2_Tracks[0][2][2]->Fill(treeline->fChi);
                ...
                case 4:
                  //for package X, front VDC, U plane
                  h_SLOPE_Tracks[treeline->GetPackage()][2][1]->Fill(treeline->GetPackage() == 1 ?
                  -treeline->fSlope : treeline->fSlope);
                  h_CHI2_Tracks[treeline->GetPackage()][2][1]->Fill(treeline->fChi);
        ...
        for(int pkg=0; pkg<c_Chi2[branch].size();pkg++)
        {
          switch(pkg)
          {
            ...
            case 2: //package 2
              ...
              {
                //name the canvases
                c_Slope[branch][pkg][VDC] = new TCanvas (Form("c_Slope[%d][%d][%d]",branch,pkg,VDC),
                  Form("Slope for %s in package %d, %s VDC",INDEXTOBRANCH[branch].c_str(), pkg, INDEXTOVDC[VDC].c_str()),
                  900,300);

                c_Chi2[branch][pkg][VDC] = new TCanvas (Form("c_Chi2[%d][%d][%d]",branch,pkg,VDC),
                  Form("Chi squared for %s in package %d, %s VDC",INDEXTOBRANCH[branch].c_str(), pkg, INDEXTOVDC[VDC].c_str()),
                  900,300);
                ...
                if(VDC==0)  //this is the both VDC combination
                {
                  ...
                  for(int plane=1; plane<=2; plane++) //graphing the UU and VV plane
                  {
                    ...
                    h_Temp_Chi2 = (TH1D*)gDirectory->
                      Get(Form("h_CHI2_%s[%d][%d][%d]",INDEXTOBRANCH[branch].c_str(),pkg,VDC,plane+2));
                    h_Temp_Chi2->Draw();

                    //draw the slope stuff
                    c_Slope[branch][pkg][VDC]->cd(plane);
                    h_Temp_Slope = (TH1D*)gDirectory->
                      Get(Form("h_SLOPE_%s[%d][%d][%d]",INDEXTOBRANCH[branch].c_str(),pkg,VDC,plane+2));
                    h_Temp_Slope->Draw();
                  }
                }else //front or back VDCs then we have both, U, and V planes
                  {
                    ...
                    for(int plane=1; plane<=3; plane++)
                    {
                      ...
                      c_Chi2[branch][pkg][VDC]->cd(plane);
                      h_Temp_Chi2 = (TH1D*)gDirectory->
                        Get(Form("h_CHI2_%s[%d][%d][%d]",INDEXTOBRANCH[branch].c_str(),pkg,VDC,plane-1));
                      if (h_Temp_Chi2 == 0)
                        cout << "Error: Histogram " << Form("h_CHI2_%s[%d][%d][%d]",INDEXTOBRANCH[branch].c_str(),pkg,VDC,plane-1) << " doesn't exist!" << endl;
                      else h_Temp_Chi2->Draw();

                      //draw the slope stuff
                      c_Slope[branch][pkg][VDC]->cd(plane);
                      h_Temp_Slope = (TH1D*)gDirectory->
                        Get(Form("h_SLOPE_%s[%d][%d][%d]",INDEXTOBRANCH[branch].c_str(),pkg,VDC,plane-1));
                      h_Temp_Slope->Draw();
                    }
                  }

                c_Slope[branch][pkg][VDC]->SaveAs(PREFIX+Form("%s_Slope_package_%d_%s_VDC.png",
                  INDEXTOBRANCH[branch].c_str(), pkg, INDEXTOVDC[VDC].c_str()));
                c_Chi2[branch][pkg][VDC]->SaveAs(PREFIX+Form("%s_Chi2_package_%d_%s_VDC.png",
                  INDEXTOBRANCH[branch].c_str(), pkg, INDEXTOVDC[VDC].c_str()));
              }
...
void Print_To_File(int run_num)
{
  ...
  ***********************************/
  ...
  fout_PartialTracks_Slope.open(PREFIX+"R3_Partial_Tracks_match_up_Slope.txt");
  ...
  for (int branch=0; branch < NUMBRANCHES; branch++)
  {
    switch(branch)
    {
      case 0: //the treelines
        fout_Treelines_Slope << "Run \t pkg \t VDC \t Plane \t NumEvents \t Value \t Error" << endl;
        fout_Treelines_Chi2 << "Run \t pkg \t VDC \t Plane \t NumEvents \t Value \t Error" << endl;
          ...
          switch(pkg)
          {
            ...
            case 2: //package 2
              ...
              for(int VDC=0;VDC<NUMVDCS;VDC++)
              {
                ...
                switch(VDC)  //this is the both VDC combination
                {
                  case 0:
                    for(int plane=3; plane<5; plane++) //graphing the UU and VV plane
                    {
                      //Slope
                      h_Temp_Slope = (TH1D*)gDirectory->
                        Get(Form("h_SLOPE_%s[%d][%d][%d]",INDEXTOBRANCH[branch].c_str(),pkg,VDC,plane));
                      ...
                      //Chi2
                      h_Temp_Chi2 = (TH1D*)gDirectory->
                        Get(Form("h_CHI2_%s[%d][%d][%d]",INDEXTOBRANCH[branch].c_str(),pkg,VDC,plane));
      ...
      case 1:  //Patial Tracks branch
        fout_PartialTracks_Slope << "Run \t pkg \t VDC \t Plane \t NumEvents \t Value \t Error" << endl;
        fout_PartialTracks_Chi2 << "Run \t pkg \t VDC \t Plane \t NumEvents \t Value \t Error" << endl;
            ...
            case 2: //package 2
              ...
              for(int VDC=0;VDC<NUMVDCS;VDC++)
              {
                ...
                switch(VDC)  //this is the both VDC combination
                {
                  ...
                  case 1:  //Front VDC - nothing here
                  case 2:  //Back VDC - nothing here
                  default:
                    break;
      ...
      case 2:  //Tracks branch
        fout_Tracks_Slope << "Run \t pkg \t VDC \t Plane \t NumEvents \t Value \t Error" << endl;
        fout_Tracks_Chi2 << "Run \t pkg \t VDC \t Plane \t NumEvents \t Value \t Error" << endl;
