 ...
 * Sept 24, 2014
 *  
 * tweaked June 13 2016 to saveAs .PDF files
 */
...
void bcm_plotter(int runNumber, int eventLow=0, int eventHigh=4e6) {
  ...
  eventLow  = (eventLow<0) ? 0 : eventLow;
  ...
  TH1F *bcm_hist = new TH1F("bcm_hist","title",1200,0,2.4);
  ...
  canvas1->SaveAs(Form("Run_%i_BCM.pdf",runNumber));
  ...
  canvas2->SaveAs(Form("Run_%i_BCM_vs_time.pdf",runNumber));

  std::cout <<"The BCM2 mean is: \t" <<bcm_hist->GetMean() <<std::endl;
