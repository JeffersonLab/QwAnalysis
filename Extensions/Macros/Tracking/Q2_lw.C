...
// from the rootfiles, and study the MD light-weighting issue;
// it looks at the LightWeighted factor for Q^2 for each packages, and 
// studies it over a range of relative gain factors for the two + and - PMT on that MD
// 
//  Also looks at gain ratio of the two PMTs.
//
// It is based on Q2_check.C, originally written by Siyuan Yang.
//
//  Lots of extra diagnostic histgrams printed, more can be uncmmented to print as well.
//
// Usage:
//
// root[0] .L Extensions/Macros/Tracking/Q2_lw.C
// root[1] q2_check(1,100000,13653)
//             ->  analyzes first 100K events of run 13653
//               
// root[1] q2_check(1,-1,13653,true)
//             -> looks at all events for Run 13653; calculates means and sigmas for matching
//                parameter distributions to base cuts on.
/
//
...
void q2_check(int event_start=-1,int event_end=-1,int run=8658, bool opt=false, TString stem="Qweak_", string suffix=""){
   ...
   double gain_factor[9] = {0.25,0.333,0.5,0.8,1.0,1.25,1.5,2.0,3.0};
   ...
   std::vector<TH1D*> q2_p1_lw;
   std::vector<TH1D*> q2_p2_lw;
   Double_t q2_p1_lw_mean[9];
   Double_t q2_p2_lw_mean[9];
   q2_p1_lw.resize(9);
   q2_p2_lw.resize(9);

   for (int jj = 0; jj<9; jj++){
     q2_p1_lw[jj] = new TH1F(Form("q2_p1_lw[%d]",jj),Form("Q2 distribution in Package 1, light-weighted factor= %f",gain_factor[jj]),100,0,0.12);
     q2_p2_lw[jj] = new TH1F(Form("q2_p2_lw[%d]",jj),Form("Q2 distribution in Package 2, light-weighted factor= %f",gain_factor[jj]),100,0,0.12);
   }
   ...
   TH1F* q2_1_lw=new TH1F("q2_1","Q2 distribution in Package 1, light-weighted",100,0,0.12);
   TH1F* q2_2_lw=new TH1F("q2_2","Q2 distribution in Package 2,light-weighted",100,0,0.12);

   TH1F* q2_1_test=new TH1F("q2_1_test","Q2 Package 1, VDC wires cut",100,0,0.12);
   TH1F* q2_1_test2=new TH1F("q2_1_test2","Q2 Package 1, VDC bad wires ",100,0,0.12);

   TH1F* q2_1_v0=new TH1F("q2_1_v0","Q2_v0 (with eloss) in Package 1",100,0,0.12);
   TH1F* q2_1_v3=new TH1F("q2_1_v3","Q2_v3 (with eloss) in Package 1",100,-.3,0.3);

   TH1F* q2_2_v0=new TH1F("q2_2_v0","Q2_v0 (with eloss) in Package 2",100,0,0.12);
   TH1F* q2_2_v3=new TH1F("q2_2_v3","Q2_v3 (with eloss) in Package 2",100,-.3,0.3);
   ...
   h_wires_raw_hit_1= new TH1F("h_wires_raw_hit_1", "Vader wire hits (raw)  V-Plane ", 280, 0.5, 280.5);
   h_wires_raw_hit_2= new TH1F("h_wires_raw_hit_2", "Vader wire hits (raw)  U-Plane ", 280, 0.5, 280.5);
   h_wires_raw_hit_3= new TH1F("h_wires_raw_hit_3", "Leia wire hits (raw)  V-Plane ", 280, 0.5, 280.5);
   h_wires_raw_hit_4= new TH1F("h_wires_raw_hit_4", "Leia wire hits (raw)  U-Plane ", 280, 0.5, 280.5);
   h_wires_raw_hit_5= new TH1F("h_wires_raw_hit_5", "Yoda wire hits (raw)  V-Plane ", 280, 0.5, 280.5);
   h_wires_raw_hit_6= new TH1F("h_wires_raw_hit_6", "Yoda wire hits (raw)  U-Plane ", 280, 0.5, 280.5);
   h_wires_raw_hit_7= new TH1F("h_wires_raw_hit_7", "Han wire hits (raw)  V-Plane ", 280, 0.5, 280.5);
   h_wires_raw_hit_8= new TH1F("h_wires_raw_hit_8", "Han wire hits (raw)  U-Plane ", 280, 0.5, 280.5);
   ...
   //try to get the oct number of where Region 3 partial tracks in package 2 are.
   ...
   int end=(event_end==-1)? nevents:event_end;
    ...
    if(opt){
      ...
      cout << "opt = true; determine matching parameters cut values " << endl;
    ...
    //  set minimum values for the sigmas of matching parameters, in case "opt" was chosen 
    // but Gaussian fit failed, or "opt" not true. 
  ...
  // this bit is to get quick ADC pedestal estimates
    ...
    cout << " Package 1 MD+ ADC pedestal = " << md_adc_p_1_ped << endl;
    cout << " Package 1 MD- ADC pedestal = " << md_adc_m_1_ped << endl;
    cout << " Package 2 MD+ ADC pedestal = " << md_adc_p_2_ped << endl;
    cout << " Package 2 MD- ADC pedestal = " << md_adc_m_2_ped << endl;
	...
	cout << "events processed so far: " << i << endl;
      ...
      // Make raw hit distributions for VDCs - no cuts. 

      for(int j=0;j<nhits;++j){
         hit=fEvent->GetHit(j);
	 if(hit->GetRegion() ==3 && hit->GetDriftDistance() >=0){
	    if (hit->GetPlane()==1 && hit->GetPackage()==1){
	      int wire = hit->GetElement();
	      h_wires_raw_hit_1->Fill(wire);
	    }
	    if (hit->GetPlane()==2 && hit->GetPackage()==1){
	      int wire = hit->GetElement();
	      h_wires_raw_hit_2->Fill(wire);
	    }
	    if (hit->GetPlane()==3 && hit->GetPackage()==1){
	      int wire = hit->GetElement();
	      h_wires_raw_hit_3->Fill(wire);
	    }
	    if (hit->GetPlane()==4 && hit->GetPackage()==1){
	      int wire = hit->GetElement();
	      h_wires_raw_hit_4->Fill(wire);
	    }
	    if (hit->GetPlane()==1 && hit->GetPackage()==2){
	      int wire = hit->GetElement();
	      h_wires_raw_hit_5->Fill(wire);
	    }
	    if (hit->GetPlane()==2 && hit->GetPackage()==2){
	      int wire = hit->GetElement();
	      h_wires_raw_hit_6->Fill(wire);
	    }
	    if (hit->GetPlane()==3 && hit->GetPackage()==2){
	      int wire = hit->GetElement();
	      h_wires_raw_hit_7->Fill(wire);
	    }
	    if (hit->GetPlane()==4 && hit->GetPackage()==2){
	      int wire = hit->GetElement();
	      h_wires_raw_hit_8->Fill(wire);
	    }
	  }
      }	
	...
	if(track->fBack->GetPackage()==1){
	  ...
	  if (valid_hits_1 < multiple && prompt_mdp1 && prompt_mdm1){
	    if(track->fDirectionPhioff>pkg1_phioff_lower && track->fDirectionPhioff<pkg1_phioff_upper && track->fDirectionThetaoff>pkg1_thetaoff_lower && track->fDirectionThetaoff<pkg1_thetaoff_upper ){
	      ...
	      // DSA Kludge: for octant 5 (180 degrees) move by 180 degrees so not at ends of histogram
	      ...
	      adc_ratio_1->Fill(log((adc_1p - md_adc_p_1_ped)/(adc_1m - md_adc_m_1_ped+.00001)));
              ...
              q2_1_lw->Fill(Q2_val,abs(adc_1p - md_adc_p_1_ped + adc_1m - md_adc_m_1_ped));
	      for (jj = 0; jj<9; jj++){
		q2_p1_lw[jj]->Fill(Q2_val,abs(gain_factor[jj]*(adc_1p - md_adc_p_1_ped) + adc_1m - md_adc_m_1_ped));
	      }
	   ...
	   if (valid_hits_2 < multiple && prompt_mdp2 && prompt_mdm2){
	     if(track->fDirectionPhioff>pkg2_phioff_lower && track->fDirectionPhioff<pkg2_phioff_upper && track->fDirectionThetaoff>pkg2_thetaoff_lower && track->fDirectionThetaoff<pkg2_thetaoff_upper){
	       ...
	       q2_2_lw->Fill(Q2_val,abs(adc_2p - md_adc_p_2_ped + adc_2m - md_adc_m_2_ped));
	       for (jj = 0; jj<9; jj++){
		q2_p2_lw[jj]->Fill(Q2_val,abs(gain_factor[jj]*(adc_2p - md_adc_p_2_ped) + adc_2m - md_adc_m_2_ped));
	      }
    ...
    double q2_1_mean= q2_1->GetMean();
    double q2_2_mean= q2_2->GetMean();
      

    for (jj = 0; jj<9; jj++){
      q2_p1_lw_mean[jj]= (q2_p1_lw[jj]->GetMean()/q2_1_mean);
      q2_p2_lw_mean[jj]= (q2_p2_lw[jj]->GetMean()/q2_2_mean);
    }
    ...
    TCanvas* craw = new TCanvas("craw","Region 3 Wire Hits (raw)",1000,500);
    craw->Divide(4,2);
    craw->cd(1);
    h_wires_raw_hit_1 -> SetLineColor(2);
    h_wires_raw_hit_1->Draw();
    craw->cd(2);
    h_wires_raw_hit_2 -> SetLineColor(2);
    h_wires_raw_hit_2->Draw();
    craw->cd(5);
    h_wires_raw_hit_3 -> SetLineColor(4);
    h_wires_raw_hit_3->Draw();
    craw->cd(6);
    h_wires_raw_hit_4 -> SetLineColor(4);
    h_wires_raw_hit_4->Draw();
    craw->cd(3);
    h_wires_raw_hit_5 -> SetLineColor(3);
    h_wires_raw_hit_5->Draw();
    craw->cd(4);
    h_wires_raw_hit_6 -> SetLineColor(3);
    h_wires_raw_hit_6->Draw();
    craw->cd(7);
    h_wires_raw_hit_7 -> SetLineColor(6);
    h_wires_raw_hit_7->Draw();
    craw->cd(8);
    h_wires_raw_hit_8 -> SetLineColor(6);
    h_wires_raw_hit_8->Draw();
    craw->SaveAs(outputPrefix+"VDC_raw_hits.pdf");
    ...
    TCanvas* ct=new TCanvas("ct","Q^2 LW various gain factor Package 1",800,600);
    ct->Divide(3,3);
    ct->cd(1);
    q2_p1_lw[0]->Draw();
    ct->cd(2);
    ...
    ct->cd(9);
    q2_p1_lw[8]->Draw();
    ct->SaveAs(outputPrefix+"Q2_gainfactors_p1.pdf");

    TCanvas* ct2=new TCanvas("ct2","Q^2 LW various gain factors, Package 2 ",800,600);
    ct2->Divide(3,3);
    ct2->cd(1);
    q2_p2_lw[0]->Draw();
    ct2->cd(2);
    ...
    ct2->cd(9);
    q2_p2_lw[8]->Draw();
    ct2->SaveAs(outputPrefix+"Q2_gainfactors_p2.pdf");
    ...
    */
    ...
    TCanvas *wires_c = new TCanvas("wires_c","Vader and Leia (Package 1)    Wires Hit when Track",450,450);
    ...
    TCanvas *wires_c2 = new TCanvas("wires_c2","Yoda and Han (Package 2)    Wires Hit when Track",500,0,450,450);
    ...
    */
    ...
    TCanvas *cgain = new TCanvas("cgain","LW factor vs. gain factor",200,10,700,900);

    cgain->cd(1);
    TGraph *gr1 = new TGraph(9,gain_factor,q2_p1_lw_mean);
    TGraph *gr2 = new TGraph(9,gain_factor,q2_p2_lw_mean);

    gPad->SetLeftMargin(0.20); 
    gStyle->SetOptFit(0000);
    gStyle->SetOptStat(0000);
    gr1->GetYaxis()->SetTitleOffset(2.0); 
    gr1->SetTitle(Form("LW factor vs. gain factor, Run %d ",run));
    gStyle->SetTitleX(0.5);
    gr1->SetMarkerStyle(20);
    gr1->SetMarkerColor(4);
    gr1->SetLineColor(4);
    gr1->GetHistogram()->SetMaximum(1.02); 
    gr1->GetHistogram()->SetMinimum(1.00); 
    gr1->GetXaxis()->SetTitle("PMT+/PMT- gain factor");
    gr1->GetYaxis()->SetTitle("LightWeight Factor"); 
    gr1->GetYaxis()->SetLabelSize(0.03);
    gr1->GetXaxis()->SetLabelSize(0.035);
   //   gStyle->SetOptFit(1111);
    gr1->Draw("AP");
    gr1->Fit("pol2");
    gr1->GetFunction("pol2")->SetLineColor(4);
    gr2->SetMarkerStyle(21);
    gr2->SetLineColor(2);
    gr2->SetMarkerColor(2);
    gr2->Fit("pol2");
    gr2->GetFunction("pol2")->SetLineColor(2);
    gr2->Draw("P");
    leg = new TLegend(0.22,0.75,0.65,0.88);
    leg->AddEntry(gr1,Form("Package 1 (MD %d)",md_1),"p"); 
    leg->AddEntry(gr2,Form("Package 2 (MD %d)",md_2),"p"); 
    leg->Draw();

    cgain->SaveAs(outputPrefix+"LW_vs_gainfactor.pdf");
...
//get a histogram (called h) of the octant cutting on region 3 and package 2 - only one octant will be returned - the one we want
