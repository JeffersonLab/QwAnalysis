// Name:   Q2_check_3pass.C
// Author: Siyuan Yang/David Armstrong
// Email:  sxyang@email.wm.edu
// Date:   Monday Jan 9th,2012
//
//    Special custom version for 3-pass (3.35 GeV) data
//    Don't use default fKinElasticWithLoss as data are inelastic
//     replace by fKinWithLoss and fKin
//
...
void q2_check(int event_start=-1,int event_end=-1,int run=8658, bool opt=false, TString stem="Qweak_", string suffix=""){
   ...
   TH1F* q2=new TH1F("Q2","Q2",100,0,0.3);
   ...
   TH1F* q2_1=new TH1F("q2_1","Q2 distribution in Package 1",100,0,0.3);
   TH1F* q2_2=new TH1F("q2_2","Q2 distribution in Package 2",100,0,0.3);
   ...
   TH1F* q2_1_lw=new TH1F("q2_1","Q2 distribution in Package 1, light-weighted",100,0,0.3);
   TH1F* q2_2_lw=new TH1F("q2_2","Q2 distribution in Package 2,light-weighted",100,0,0.3);
   ...
   TH1F* q2_1_v0=new TH1F("q2_1_v0","Q2_v0 (with eloss) in Package 1",100,0,0.3);
   TH1F* q2_2_v0=new TH1F("q2_2_v0","Q2_v0 (with eloss) in Package 2",100,0,0.3);
   TH1F* eprime_1=new TH1F("eprime_1","E' in Package 1",120,0.6,1.6);
   TH1F* eloss_1=new TH1F("eloss_1","E (with eloss) in Package 1",50,3.0,3.4);
   TH1F* eprime_2=new TH1F("eprime_2","E' in Package 2",120,0.6,1.6);
   TH1F* eloss_2=new TH1F("eloss_2","E (with eloss) in Package 2",50,3.0,3.4);
  ...
  TChain* chain = new TChain("event_tree");
   ...
   cout << "start event = " << start << "   end event = "  << end << endl;

   event_tree->SetBranchStatus("events",1);
 
 //       event_tree->SetBranchStatus("events",0);
  ...
  // this bit is to get quick ADC pedestal estimates
    ...
    */
	...
	if(track->fBack->GetPackage()==1){
	  ...
	  if (valid_hits_1 < multiple && (prompt_mdp1 || prompt_mdm1)){
	    if(track->fDirectionPhioff>pkg1_phioff_lower && track->fDirectionPhioff<pkg1_phioff_upper && track->fDirectionThetaoff>pkg1_thetaoff_lower && track->fDirectionThetaoff<pkg1_thetaoff_upper ){
	      ...
	      cout << "   event number =  " << i << "   W_val = " << W_val <<  "   n_pt_r2andr3_p1 = " << n_pt_r2andr3_p1  << endl; 
	      ...
	      double Q2_zero_1 = fEvent->fKin.fQ2; 
	   ...
	   if (valid_hits_2 < multiple && (prompt_mdp2 || prompt_mdm2)){
	     if(track->fDirectionPhioff>pkg2_phioff_lower && track->fDirectionPhioff<pkg2_phioff_upper && track->fDirectionThetaoff>pkg2_thetaoff_lower && track->fDirectionThetaoff<pkg2_thetaoff_upper){
	       ...
	       double Q2_zero_2 = fEvent->fKin.fQ2; 
    ...
    q2_1_v0->SetTitle("Q2, no target eloss in Package 1");
    cv->cd(2);
    q2_1->Draw();
    q2_1->GetXaxis()->SetTitle("Q2: (GeV/c)^2");
    q2_1->SetTitle("Q2, target eloss in Package 1");
    cv->cd(3);
    q2_2_v0->Draw();
    q2_2_v0->GetXaxis()->SetTitle("Q2: (GeV/c)^2");
    q2_2_v0->SetTitle("Q2, no target eloss in Package 2");
    cv->cd(4);
    q2_2->Draw();
    q2_2->GetXaxis()->SetTitle("Q2: (GeV/c)^2");
    q2_2->SetTitle("Q2, target eloss Package 2");
    ...
    eloss_1->SetTitle("E_0 with Eloss in Package 1");
    ce->cd(3);
    eprime_2->Draw();
    ...
    eloss_2->SetTitle("E_0 with Eloss in Package 2");
    ce->SaveAs(outputPrefix+"Eprimes.pdf");

    TCanvas* cw=new TCanvas("cw","W2, nu,y, x  ",800,600);
    cw->Divide(4,2);
    cw->cd(1);
    gStyle->SetStatW(0.3);                
    ...
    W2_1->Draw();
    W2_1->GetXaxis()->SetTitle("W^2 (GeV)^2)");
    W2_1->SetTitle("W^2 in Package 1");
    cw->cd(5);
    W2_2->Draw();
    W2_2->GetXaxis()->SetTitle("W^2 (GeV)^2");
    W2_2->SetTitle("W^2 in Package 2");
    cw->cd(2);
    W_1->Draw();
    W_1->GetXaxis()->SetTitle("W (GeV)");
    W_1->SetTitle("W in Package 1");
    cw->cd(6);
    W_2->Draw();
    W_2->GetXaxis()->SetTitle("W (GeV)");
    W_2->SetTitle("W in Package 2");
    cw->cd(3);
    nu_1->Draw();
    nu_1->GetXaxis()->SetTitle("nu (GeV)");
    nu_1->SetTitle("nu in Package 1");
    cw->cd(7);
    nu_2->Draw();
    nu_2->GetXaxis()->SetTitle("nu (GeV)");
    nu_2->SetTitle("nu in Package 2");
    cw->cd(4);
    x_1->Draw();
    x_1->GetXaxis()->SetTitle("Bjorken x");
    x_1->SetTitle("Bjorken x in Package 1");
    cw->cd(8);
    x_2->Draw();
    x_2->GetXaxis()->SetTitle("Bjorken x");
    x_2->SetTitle("Bjorken x in Package 2");
    cw->SaveAs(outputPrefix+"W2.pdf");
