	      ...
	      TTree *tree, Double_t lasLlimit = 20000.0, Bool_t isFirst100k = kFALSE)
{
  ...
  Double_t laser, bcm;
  Double_t beamMax = tree->GetMaximum("yield_sca_bcm6");
  ...
  tree->SetBranchStatus("yield_sca_laser_PowT",1);//turn on cavity power branch
  tree->SetBranchStatus("yield_sca_bcm6",1);//turn on bcm branch
  tree->SetBranchAddress("yield_sca_laser_PowT",&laser);
  tree->SetBranchAddress("yield_sca_bcm6",&bcm);
  ...
  for(Int_t i=0; i<nEntries;i++){
    ...
    if(n==minEntries){
      cutL.push_back(i-minEntries);
      printf("cutL[%d]=%d\n",m,cutL.back());
      flipperIsUp = kTRUE;
      m++;
    }

    if(laser<=lasLlimit)n++;
    else n=0;
    ...
    rampIsDone = bcm > 0.9 * beamMax;//beam above 90% maximum
    isABeamTrip = bcm <= 1.0;
	...
	q = 0;
	o++;
	cutE.push_back(TMath::Max((i-12), 0));
	prevTripDone = kFALSE;
      }
    }
	...
	if(h->GetRMS()<3.0)//trip is over only if beam has stabilized
	{
	  o++;
	  cutE.push_back(i);
	  prevTripDone = kTRUE;
	      ...
	      Int_t runNum, Double_t lasLlimit = 5000.0, Bool_t isFirst100k = kFALSE)
{
  ...
  Double_t beamOnVal = 0.95 * beamMax;
  printf("Ebeam considered On if above %f.\n", beamOnVal);
  delete h;
  ...
  while(fileNum<numFiles){
	...
	if(q>10){
	  q = 0;
	  o++;
	  cutE.push_back(TMath::Max((z-12), 0));
	  prevTripDone = kFALSE;
	}
      }
...
/*
TChain *ch= new TChain("Hel_Tree")
ch->Add("/net/cdaq/cdaql8data/compton/rootfiles/Compton_Pass1_22737*")
std::vector<Int_t> cutE;
std::vector<Int_t> cutL;
.L ~/users/jonesdc/macros/getCuts.C+
Int_t v = getCuts(cutL,cutE,ch)
