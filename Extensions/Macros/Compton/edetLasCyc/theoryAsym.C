...
Float_t theoryAsym() 
{
  ...
  if(debug2) cout<<"\nStrip#\tdist(mm)\trho\tk-photon\tasymStrip"<<endl;
  
for(Int_t s =0; s <=stripNum; s++) {
    kStripprime=((xStrip+s*2E-4)/zdrift)*E/thetabend; //   scattered photon energy (GeV)
    ...
    asymStrip = (2*pi*re*re/100*a/dsdrho)*(1-rhoStrip*(1+a))*(1-1/(asymStrip1*asymStrip1)); //
  
    cout<<s<<"\t"<<(xStrip+s*1E-4)*1000<<"\t"<<rhoStrip<<"\t"<<kStripprime<<"\t"<<asymStrip<<endl;
  }
  return asymStrip;
}
