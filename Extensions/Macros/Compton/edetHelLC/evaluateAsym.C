  ...
  for (Int_t s =startStrip; s <endStrip; s++) {	  
    ...
    if(BCqNormSumLC>0.0) {
      ...
      if (asymErSqrLC[s] >0.0) {///eqn 4.17(Bevington)
        ...
        cout<<"check if strip "<<s+1<<" is MASKED? It gives non-positive Asym Er"<<endl;
      ...
      if (erBCqNormSumLCSq >0.0) {
        ...
        wmNrqNormB1L0[s] += qNormAcBkgdAsymDr/erqNormB1L0LasCycSq; //refer bevington
