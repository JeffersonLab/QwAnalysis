...
Int_t plotScalerFile(Int_t runStart=20300, Int_t runEnd=20409)
{
  ...
  TGraphErrors *gr = new TGraphErrors(j,cntr,pD_ScaBkgd,errorX,pD_ScaBkgdErr);
  gr->GetXaxis()->SetTitle("Cut Number");
  gr->GetYaxis()->SetTitle("Scaler Asymmetry (%)");
  gr->GetYaxis()->SetTitleOffset(1.2);
  gr->SetTitle("PD Average Laserwise Background");
  ...
  gr->Draw("AP");
  c1->cd(2);
  TGraphErrors *gr1 = new TGraphErrors(j,cntr,scintScaBkgd,errorX,scintScaBkgdErr);
  gr1->GetXaxis()->SetTitle("Cut Number");
  gr1->GetYaxis()->SetTitle("Scint Avg Laserwise Bkgd Factor");
  gr1->GetYaxis()->SetTitleOffset(1.2);
  gr1->SetTitle("Scint Avg Bkgd Factor");
