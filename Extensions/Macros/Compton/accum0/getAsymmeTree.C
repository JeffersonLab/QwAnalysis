...
//create useful variables
  ...
  for(Int_t i=0;i<nEntries;i++){
	...
	if(flipperIsUp){
	  if(hPedOff->GetEntries()>MIN_ENTRIES){//only store if sufficient data
	    ...
	    printf("Laser OFF:  lasWisePedOff[%i]= %1.0f. Insufficient OFF data:  %d entries\n"
		   ...
		   "Begin Laser On: Entry = %i\n",
		   nCycles*2, lasWisePedOff[nCycles*2],(Int_t)hPedOff->GetEntries(),
	...
	if(nCycles!=0 && hPedOn->GetEntries()>MIN_ENTRIES){
	  ...
	  printf("Laser ON:   lasWisePedOn[%i]= %11.4e +/- %7.1e.\n"
	  ...
	  lasWisePedOnErr[nCycles-1] = hPedOn->GetRMS()
	                        /TMath::Sqrt((Double_t)hPedOn->GetEntries());
	  printf("Laser ON:   lasWisePedOn[%i] = %1.0f. Insufficient ON data.%f entries\n"
		 "          lasWiseYieldOn[%i] = %1.0f. .                    %f entries\n" 
		 "Begin Laser Off: Entry = %i\n",
		 nCycles-1, lasWisePedOn[nCycles-1],hPedOn->GetEntries(),
		 nCycles-1, lasWiseYieldOn[nCycles-1],hYieldOn->GetEntries(),i); 
	}
	...
	lasWiseBkgdErr[nCycles] = hBackground->GetRMS()
	  /TMath::Sqrt((Double_t)hBackground->GetEntries());
	printf("Laser OFF:  lasWisePedOff[%i]= %11.4e +/- %7.1e.\n"
	       "           lasWiseBkgd[%i]= %11.4e +/- %7.1e.\n"
	       "Final Laser Off: Entry = %i\n",
	       nCycles, lasWisePedOff[nCycles],lasWisePedOffErr[nCycles],
	       nCycles, lasWiseBkgd[nCycles],lasWiseBkgdErr[nCycles],i);
      }else{
	lasWisePedOff[nCycles] = -9999; 
	lasWisePedOffErr[nCycles] = -9999;
	lasWiseBkgd[nCycles] = -9999;
  ...
  printf("There were %i good cycles.\n", nGoodCycles);
  ...
  for(Int_t i=0;i<nEntries;i++){
	...
	if(pedAdjBkgd[nCycles]!=0){
	asymmetry = yieldAccum0*asymmAccum0/(yieldAccum0-pedAdjBkgd[nCycles]);
	...
	errorY[nCycles-1] = 100*hAsymm->GetRMS()/TMath::Sqrt(hAsymm->GetEntries());
	printf("asymm[%i]=%0.5f%%  (+/- %0.4f%%).\n",
	       nCycles-1,asymm[nCycles-1],errorY[nCycles-1]);
	fprintf(pedAdjStats,"%d\t%12.5e\t%9.2e\t%d\t%12.5e\t%12.5e\t%9.2e\t%12.5e\t%9.2e"
		"\t%12.5e\t%9.2e\t%12.5e\t%9.2e\n",
		nCycles-1,asymm[nCycles-1], errorY[nCycles-1],(Int_t)hAsymm->GetEntries(),
