...
Int_t readPedFile(Int_t runStart=20300, Int_t runEnd=20409, Bool_t wwwdon=kFALSE)
{
  ...
  Double_t  asymm[LEN], asymmRMS[LEN], yob[LEN], yield[LEN], yieldRMS[LEN];
  Double_t bkgd[LEN], bkgdRMS[LEN], pedShift[LEN], pedShiftRMS[LEN];
  Double_t pedAdjBkgd[LEN], pedAdjBkgdRMS[LEN];
  Double_t errorX[LEN], errorY[LEN], cntr[LEN];
  ...
  {
    runnum = i;// runs[i];
    ifstream file(Form("wwwdon/Run_%i/Run_%i_pedAdjStats.txt",runnum, runnum));
    ...
    {    
      ...
      while(file>>cutNum[j]>>asymm[j]>>asymmRMS[j]>>numEnt[j]>>yob[j]>>yield[j]
	    >>yieldRMS[j]>>bkgd[j]>>bkgdRMS[j]>>pedShift[j]>>pedShiftRMS[j]
	    >>pedAdjBkgd[j]>>pedAdjBkgdRMS[j])
      {
	...
	if(debug)
	{
	  cout<<"cutNum="<<cutNum[j]<<", asymm="<<asymm[j]<<", asymmRMS="
	      ...
	      <<", yield="<<yield[j]<<", yieldRMS="<<yieldRMS[j]<<",\n"
	      <<"bkgd="<<bkgd[j]<<", bkgdRMS="<<bkgdRMS[j]<<",\n"<<"pedShift="
	      <<pedShift[j]<<", pedShiftRMS="<<pedShiftRMS[j]<<", pedAdjBkgd="
	      <<pedAdjBkgd[j]<<", pedAdjBkgdRMS="<<pedAdjBkgdRMS[j]<<"\n"<<endl;
	}
	cntr[j] = j;
  ...
  TGraphErrors *gr = new TGraphErrors(j, pedShift, asymm, errorX, errorY);
  ...
  gr->SetTitle(Form("Background Subtracted Acumm0 Asymmetry vs Yield above"
		    " Background  (Runs %i-%i)", runStart, runEnd));
