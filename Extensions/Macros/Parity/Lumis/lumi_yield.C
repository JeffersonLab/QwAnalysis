...
const string ds_offsets[9] = 
{
"0","0","0",
"0","","0",
"0","0","0"};

const string us_offsets[16] = 
{
"","0","0","",
"0","","","0",
"0","","","0",
      ...
      if (view_mps == kTRUE)
        {
          if (view_volt == kTRUE) 
            {
              chain->Draw(Form("(yield_qwk_%s.hw_sum-%s)*0.00007629:mps_counter*%f>>run%s_h_%s",DS[p],ds_offsets[p],scale_mps,run_num,DS[p]),Form("%s && %s",cut1,cut2),"");      
            }
          else 
            {
              chain->Draw(Form("yield_qwk_%s.hw_sum-%s:mps_counter*%f>>run%s_h_%s",DS[p],ds_offsets[p],scale_mps,run_num,DS[p]),Form("%s && %s",cut1,cut2),""); 
            }
        }
      else
        { 
          if (view_volt == kTRUE)
            {             
              chain->Draw(Form("(yield_qwk_%s.hw_sum-%s)*0.00007629>>run%s_h_%s",DS[p],ds_offsets[p],run_num,DS[p]),Form("%s && %s",cut1,cut2),""); 
            }
          else 
            {
              chain->Draw(Form("yield_qwk_%s.hw_sum-%s>>run%s_h_%s",DS[p],ds_offsets[p],run_num,DS[p]),Form("%s && %s",cut1,cut2)); 
            }
        }
...
if (save_file == kTRUE && view_mps == kTRUE)
  {
    mc_ds->SaveAs(Form("$QWSCRATCH/rootfiles/run%s_yield_dslumis_mps.png",run_num));
    mc_ds->SaveAs(Form("$QWSCRATCH/rootfiles/run%s_yield_dslumis_mps.ps",run_num));
  }
if (save_file == kTRUE && view_mps == kFALSE)
  {
    mc_ds->SaveAs(Form("$QWSCRATCH/rootfiles/run%s_yield_dslumis.png",run_num));
    mc_ds->SaveAs(Form("$QWSCRATCH/rootfiles/run%s_yield_dslumis.ps",run_num));
  }
      ...
      if (view_mps == kTRUE)
        {
          if (view_volt == kTRUE) 
            {
              chain->Draw(Form("(yield_qwk_%s.hw_sum-%s)*0.00007629:mps_counter*%f>>run%s_h_%s",US[p],us_offsets[p],scale_mps,run_num,US[p]),Form("%s && %s",cut1,cut2),"");      
            }
          else 
            {
              chain->Draw(Form("yield_qwk_%s.hw_sum-%s:mps_counter*%f>>run%s_h_%s",US[p],us_offsets[p],scale_mps,run_num,US[p]),Form("%s && %s",cut1,cut2),""); 
            }
        }
      else
        {
          if (view_volt == kTRUE)
            {             
              chain->Draw(Form("(yield_qwk_%s.hw_sum-%s)*0.00007629>>run%s_h_%s",US[p],us_offsets[p],run_num,US[p]),Form("%s && %s",cut1,cut2),""); 
            }
          else 
            {
              chain->Draw(Form("yield_qwk_%s.hw_sum-%s>>run%s_h_%s",US[p],us_offsets[p],run_num,US[p]),Form("%s && %s",cut1,cut2)); 
            }
        }
...
if (save_file == kTRUE && view_mps == kTRUE)
  {
    mc_us->SaveAs(Form("$QWSCRATCH/rootfiles/run%s_yield_uslumis.png",run_num));
    mc_us->SaveAs(Form("$QWSCRATCH/rootfiles/run%s_yield_uslumis.ps",run_num));
  }
if (save_file == kTRUE && view_mps == kFALSE)
  {
    mc_us->SaveAs(Form("$QWSCRATCH/rootfiles/run%s_yield_uslumis.png",run_num));
