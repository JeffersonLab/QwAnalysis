...
void QwGUIEventDisplay::MakeLayout()
{
   ...
   fMain = new TGVerticalFrame(this);
...
void QwGUIEventDisplay::GoClear()
{
...
/*fit = sum/num;
  ...
  for(QwHitContainer::iterator fHit = Hits_R2u->begin(); fHit != Hits_R2u->end(); fHit++){
    ...
    if (fWire < 12){
      ...
      Line.SetY2(.5 - (R2_LENGTH*R2_CM*.5) + (R2_UVDIST*R2_CM*fWire));
      Line.SetX1(.5 + (R2_WIDTH*R2_CM*.5) - (Line.GetY2()*tan(R2_ANGLE*TMath::DegToRad())) - fXShift);
      Line.SetX2(.5 + (R2_WIDTH*R2_CM*.5) - fXShift);
    }
    else{
      Line.SetX1(.5 - (R2_WIDTH*R2_CM*.5));  /// incorporate fXShift when I figure it out!!
      Line.SetX2(.5 + (R2_WIDTH*R2_CM*.5) - (R2_UVDIST*R2_CM*(fWire-12)));
      Line.SetY1(((.5 + (R2_WIDTH*R2_CM*.5)) - (Line.GetX2())*tan(R2_ANGLE*TMath::DegToRad())));
...
/*
 ...
 //Region 2 U wires (29 total)
  ...
  for(QwHitContainer::iterator fHit = Hits_R3u->begin(); fHit != Hits_R3u->end(); fHit++){
    ...
    TLine Line;
    if (fWire < 54){  //96th wire ends at corner (X2, Y2)
    ...
    else if (fWire < 185){  //184th wire begins at (X1, Y1)
      Line.SetX1(.5 + (R3_WIDTH*R3_CM*.5) - (R3_UVDIST*R3_CM*fWire));
      Line.SetY1(fYShift - (R3_LENGTH*R3_CM*.5));
      Line.SetX2(.5 + (R3_WIDTH*R3_CM*.5) - (R3_UVDIST*R3_CM*(fWire-53.6)));
      Line.SetY2(fYShift + (R3_LENGTH*R3_CM*.5));
    }
    else{
      Line.SetX1(.5 - (R3_WIDTH*R3_CM*.5));
      Line.SetY1(fYShift - (R3_LENGTH*R3_CM*.5) + (R3_UVDIST*R3_CM*(fWire-185)));
      Line.SetX2(.5 + (R3_WIDTH*R3_CM*.5) - (R3_UVDIST*R3_CM*(fWire-53.6))); // ISSUES HERE!  FIXXX!!
      Line.SetY2(fYShift + (R3_LENGTH*R3_CM*.5));
    }
    Line.SetLineColor(kGreen);
    Line_R2u.push_back(Line);
  ...
  for(QwHitContainer::iterator fHit = Hits_R3v->begin(); fHit != Hits_R3v->end(); fHit++){
    ...
    TLine Line;
    if (fWire < 54){  //96th wire ends at corner (X2, Y1)
      Line.SetX1(.5 + (R3_WIDTH*R3_CM*.5) - (R3_UVDIST*R3_CM*fWire));
      Line.SetY1(fYShift + (R3_LENGTH*R3_CM*.5));
      Line.SetX2(.5 + (R3_WIDTH*R3_CM*.5));
      Line.SetY2(fYShift + (R3_LENGTH*R3_CM*.5) - (R3_UVDIST*R3_CM*fWire*tan(R3_ANGLE*TMath::DegToRad())));
    }
    else if (fWire < 185){  //184th wire begins at corner (X1, Y2)
      Line.SetX1(.5 + (R3_WIDTH*R3_CM*.5) - (R3_UVDIST*R3_CM*fWire));
      Line.SetY1(fYShift + (R3_LENGTH*R3_CM*.5));
      Line.SetX2(.5 + (R3_WIDTH*R3_CM*.5) - (R3_UVDIST*R3_CM*(fWire-53.6)));
      Line.SetY2(fYShift - (R3_LENGTH*R3_CM*.5));
