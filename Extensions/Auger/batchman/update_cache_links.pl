...
foreach $sourcefile (`find $source_directory -maxdepth 1`){
    chomp $sourcefile;
    ...
    if ($segment ne ""){
	$link_path = "$link_directory/QwRun_$run_number.log.$segment";
    } else {
	$link_path = "$link_directory/QwRun_$run_number.log";
    }
    if (-l $link_path){
...
sub displayusage {
	...
	"\tupdate_cache_links.pl [-S <source directory>]\n\n",
	"Options:\n",
	"\t-h\n",
