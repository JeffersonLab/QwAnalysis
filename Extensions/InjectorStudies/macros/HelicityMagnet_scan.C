...
// 2011-3-15  B. Waidyawansa
// Updated for helicity magnet scan plots.

void plot_element(TPad *p1, TPad* p2, char *devnam, TString lcut, Int_t dset);

void HM_scan(Int_t runnumber=1, Int_t ihwp=-1, TString device="0l05", TString usercut ="1", TString intitle="HM") {
	if (runnumber==1) {
		printf("Usage:\n\t.x HelicityMagent_scan.C(runnum,  ihwp,[device],[cut],[title])\n");
		printf("OR\t.x HelicityMagent_scan.C(filename,ihwp,[device],[cut],[title])\n\n");
		printf("defaults:\n\t[device] = \"0l05\"\n\t[cut]    = \"1\"\n\t[title]  = \"PITA\"\n\n");
		return;
	}
	TString infilename = Form("$QW_ROOTFILES/qwinjector_%i.000.root",runnumber);
	HM_scan(infilename, ihwp, device, usercut, intitle, runnumber);
}

void HM_scan(TString infilename = "1", Int_t ihwp=-1, TString device="0l05", 
...
void plot_element(TPad *p1, TPad* p2, char *devnam, TString localcut, TString infilename) {
    ...
    if (tmpname.Contains("X") || tmpname.Contains("Y")) {
		...
		plotcommand += ".hw_sum*1000"; // convert to micro meters
		titpre = "diff_";
		...
		if (tmpname.Contains("EffectiveCharge")) {
			plotcommand = "asym_";
			plotcommand += tmpname.Data();
			plotcommand += ".hw_sum*1000000"; // convert to ppm
