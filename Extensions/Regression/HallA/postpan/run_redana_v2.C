...
//for (int i=0;i<4;i++)
{
  gSystem->Exec(Form("./redana_%03i -r %i > $PAN_LOG_FILE_PATH/reg_out%i_%03i.log",segment[i],run[i],run[i],segment[i]));
  gSystem->Exec(Form("mv -f $PAN_ROOT_FILE_PATH/Qweak_%iregress_Hel_Tree.root $PAN_ROOT_FILE_PATH/reg_HallA_Qweak_%i.%03i.root",run[i],run[i],segment[i]));
  gSystem->Exec(Form("mv -f Qweak_%iregress.res $PAN_LOG_FILE_PATH/Qweak%i_%03iregress.res",run[i],run[i],segment[i]));
}
