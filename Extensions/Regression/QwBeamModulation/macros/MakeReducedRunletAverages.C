...
{
  int val = ((int(num + 0.5) > int(num)) ? int(num+0.5) : int(num));
  return val;
}


Int_t MakeReducedRunletAverages(int start = 0,  int run_period = 1, int nRunlets = 20)
{
  Bool_t debug = 0;
  ...
  TFile *DBfile = TFile::Open(Form("/net/data2/paschkelab2/DB_rootfiles/run%i/"
				   "HYDROGEN-CELL_offoff_tree.root", run_period));
  if(DBfile==0){
  ...
  for(int irunlet = start;irunlet<end;++irunlet){
    ...
    ofstream file(Form("%srunlet_averages/reduced_averages%i_%03i.dat",
		       reducedFileDir.Data(), run_number, rnlt));
    printf("%srunlet_averages/reduced_averages%i_%03i.dat",
	   reducedFileDir.Data(), run_number, rnlt);
    ...
    if(slug_number!=prev_slug){
      ...
      if(reducedFile==0){
	cout<<"Reduced slugfile not found. Exiting.\n";
	//	return -1;
      }
	...
	//if(ileaf==0)cout<<i<<" "<<list->GetEntry(i)<<" "<<leaf[ileaf]<<endl;
...
//g++ -Wall `root-config --ldflags --libs --cflags` -O0 -g //
//MakeReducedRunletAverages.C -o MakeReducedRunletAverages //
/////////////////////////////////////////////////////////////

Int_t main(int argc, char *argv[]){
  if (argc < 1 || argc > 4){
    std::cout<<"Usage:MakeReducedRunletAverages(int start, int run_period, int runlets)"
	<<std::endl;
    return 0;
  }else if (argc==4){
    MakeReducedRunletAverages(atoi(argv[1]),atoi(argv[2]),atoi(argv[3]) );
  }else if (argc==3){
    MakeReducedRunletAverages(atoi(argv[1]),atoi(argv[2]));
  }else if (argc==2){
    MakeReducedRunletAverages(atoi(argv[1]));
  }else{
    MakeReducedRunletAverages();
  }
  return 1;
}
