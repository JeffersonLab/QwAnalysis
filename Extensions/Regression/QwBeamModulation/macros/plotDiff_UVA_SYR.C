...
void plotDiff_UVA_SYR(Int_t start = 13842, Int_t end = 19000, 
			  Int_t saveToFile = 0)
{
  ...
  /////////////////////////////////////////////////////////
  TFile fSlSYR(Form("%s/../Syracuse_slope_tree_original_monitors_20140616.root",
		 gSystem->Getenv("BMOD_OUT")));
  TTree *slSYR = (TTree*)fSlSYR.Get("mps_slug");   

  TFile fSlUVA(Form("%s/../MacrocycleSlopesTree_ChiSqMin.set0.root",
		 gSystem->Getenv("BMOD_OUT")));
  TTree *slUVA = (TTree*)fSlUVA.Get("slopes"); 
  ...
  slSYR->SetBranchAddress("good",&good);
  slSYR->SetBranchAddress("run",&run);
  slUVA->SetBranchAddress("good",&mGood);
  slUVA->SetBranchAddress("run",&mRun);
  for(int idet=0;idet<nDet;++idet){
    for(int imon=0;imon<nMON;++imon){
      slSYR->SetBranchAddress(Form("%s_%s",DetectorList[idet].Data(),
				    MonitorList[imon].Data()), 
			       &slope[idet][imon]);
      slUVA->SetBranchAddress(Form("%s_%s",DetectorList[idet].Data(),
				    MonitorList[imon].Data()), 
			       &mSlope[idet][imon]);
    }
  }
  ...
  for(int i=0;i<size;++i){
    ...
    //slopes tree
    vector< vector< vector <Double_t> > >vSlopeSYR;
    vSlopeSYR.resize(nDet);
    for(int idet=0;idet<nDet;++idet){
      vSlopeSYR[idet].resize(nMON);
    }
    while(run <= vLast[i] && ent<slSYR->GetEntries()){
      //      if(run>=vFirst[i] && good){
      if(run>=vFirst[i]){
	for(int idet=0;idet<nDet;++idet){
	  for(int imon=0;imon<nMON;++imon){
	    Double_t slp = (slope[idet][imon]==-99999 ? 0 : slope[idet][imon]);
	    vSlopeSYR[idet][imon].push_back(slp);
	  }
	}
      }
      ++ent;
    ...
    while(mRun <= vLast[i] && mEnt<slUVA->GetEntries()){
	...
	Double_t *temp_slope = vSlopeSYR[idet][imon].data();
	Double_t *temp_m_slope = v_m_slope[idet][imon].data();
	FindAverageAndError(temp_slope, temp, vSlopeSYR[idet][imon].size());
	...
	if(debug&&idet==imon&&imon==0)
	  cout<<DetectorList[idet].Data()<<"_"<<MonitorList[imon].Data()<<"("<<
	    ...
	    vSlopeSYR[idet][imon].size()<<" "<<v_m_slope[idet][imon].size()<<
