...
void makeMacrocycleSlopesTreeNoResiduals(char *config = "", char *output = "", 
...
//     printf("%i: %i   %i\n", i, vHWP[0][i],vHWP[1][i]);
  ...
  char *output_dir = Form("%s", gSystem->Getenv("BMOD_OUT"));
  if( output != "")
    output_dir = output;
  ...
  ////////////////////////////
  newfile = new TFile(Form("/net/data1/paschkedata1/MacrocycleSlopesTree%s.root", 
			   name),"recreate");
  ////////////////////////////
