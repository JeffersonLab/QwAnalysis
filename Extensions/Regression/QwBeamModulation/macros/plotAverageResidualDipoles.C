...
void plotAverageResidualDipoles(int start = 139, int end = 321, int cos = 1){
  ...
  for(int idet=0;idet<nDET;idet++){
	...
	err[imod][idet] = h->GetMeanError();
      ...
      gr[idx]->Draw("ap");
      gr[idx]->SetTitle(Form("Residual %s Response to Coil %i (%smod)",
			     (cos ? "Cosine":"Sine"),imod, modType[imod]));
