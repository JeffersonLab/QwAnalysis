...
Int_t MakeDBrootfile(Int_t run_period = 1)
{
  Bool_t debug = 0;
  ...
  vector<Int_t>vRun, vRunlet, vSlug, vErrFlag, vMissing, vMissSlug;
  ...
  for(int irunlet=0;irunlet<DBtree->GetEntries();++irunlet){
    ...
    ifstream rnltAvgFile(Form("%srunlet_averages/reduced_averages%i_%03i.dat",
			      reducedFileDir.Data(), run_number,rnlt));
    if(!(rnltAvgFile.is_open() && rnltAvgFile.good())){
      cout<<Form("%srunlet_averages/reduced_averages%i_%03i.dat NOT found.\n",
		 reducedFileDir.Data(), run_number,rnlt);
      for(int ileaf=0;ileaf<nLeaves;++ileaf){ 
	leaf_t lf;
	lf.value = ERROR;
...
//     cout<<irunlet<<Form(":  reduced_averages%i_%03i.dat found.\n",
// 		   run_number, rnlt);
  ...
  TTree *newTree = new TTree("reduced_tree", "reduced_tree"); 
  Int_t slug, runlet, run, good;
  ...
  cout<<"vleaf.size(): "<<vLeaf[0].size()<<endl;
  int prev = -1;
