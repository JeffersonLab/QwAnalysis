...
std::vector<MyR2Pkg2_t> GetR2Pkg2(Int_t run, Int_t pass, TString path)
{
//  TString filename = TString("path")+Form("Angle_%d.txt",pass);
  TString filename = TString(gSystem->Getenv("VALERIAN")) + Form("/data/pass") + TString(gSystem->Getenv("PASS")) + Form("/OctantDetermination_R2_pkg2/OctantDetermination_R2_pkg2_%d.txt",run);
