...
//  TString filename = TString("path")+Form("Angle_%d.txt",pass);
  TString filename = TString(gSystem->Getenv("VALERIAN")) + Form("/data/pass") + TString(gSystem->Getenv("PASS")) + Form("/PhiProjMatch/PhiProjMatch_%d.txt",run);
...
//  if(!angle_data.is_open()) std::cout << "Angle File " << run << " not open" << std::endl;
  ...
  std::vector<MyQ2PhiMatchProj_t> phi;
  MyQ2PhiMatchProj_t tmp;
  ...
  *****************************/
  ...
  while (phi_data >> tmp.package  //read in the pkg
    && phi_data >> tmp.Val  //read in the value of phi match projected
    && phi_data >> tmp.error  //read in the error on phi match projected
