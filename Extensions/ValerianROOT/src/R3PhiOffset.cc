...
\brief <b>Purpose:</b> This file gets the number
  R3 Phi offset of  bar information
...
//  TString filename = TString("path")+Form("Angle_%d.txt",pass);
  TString filename = TString(gSystem->Getenv("VALERIAN")) + Form("/data/pass") + TString(gSystem->Getenv("PASS")) + Form("/R3_offsets_Phi/R3_offsets_Phi_%d.txt",run);
