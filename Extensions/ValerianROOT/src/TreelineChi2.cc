...
//  TString filename = TString("path")+Form("Angle_%d.txt",pass);
  TString filename = TString(gSystem->Getenv("VALERIAN")) + Form("/data/pass") + TString(gSystem->Getenv("PASS")) + Form("/R3_Treeline_match_up_Chi2/R3_Treeline_match_up_Chi2_%d.txt",run);
...
//  if(!angle_data.is_open()) std::cout << "Angle File " << run << " not open" << std::endl;
  ...
  std::vector<MyTreelineChi2_t> tchi;
  MyTreelineChi2_t tmp;
