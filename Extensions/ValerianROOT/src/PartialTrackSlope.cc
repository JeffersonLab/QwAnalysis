...
//  TString filename = TString("path")+Form("Angle_%d.txt",pass);
  TString filename = TString(gSystem->Getenv("VALERIAN")) + Form("/data/pass") + TString(gSystem->Getenv("PASS")) + Form("/R3_Partial_Tracks_match_up_Slope/R3_Partial_Tracks_match_up_Slope_%d.txt",run);
...
//  if(!angle_data.is_open()) std::cout << "Angle File " << run << " not open" << std::endl;
  ...
  *****************************/
  ...
  //while I...
  while (ptslope_data >> tmp.package  //read in the pkg
    ...
    && ptslope_data >> tmp.nevents  //read in the error
