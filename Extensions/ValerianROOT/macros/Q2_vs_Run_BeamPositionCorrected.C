/***********************************************************
 ...
 you who like them)
 3) To plot the Q2 (all versions) vs run numbercorrected for beam position.
 ...
 happy ROOT means a happy life. (However as I wrote it I am bias)

 This macro corrects for the beam position. It uses run I for the "nominal"
 beam position. This is done for each pkg individually, and by using the angle values
 and then it is applied the Q^2 values.
 - Correct for run NOMINAL_RUN to be in the nominal beam position
 - Beam position determined by the scattering angle value. These are in my
 spreadsheet in the google doc, Position_Scan_Q2_ScatteringAngle
 - The Q2 angel values are from tracking elog https://qweak.jlab.org/elog/Tracking/83
 - After getting the beam position then the Q2 is corrected to "nominal" beam
 position, which will be for run i
 - The error is not corrected for.

Definition
 - Q2WithLoss is includes the energy loss in the target and does not include
 E' in the calculation.
 - Q2WithOut doesn't include the energy loss in the target and includes E' in
 Q2 calculation.
...
std::vector<std::vector<std::vector<Double_t> > > Q2_VAL_CORRECTED;  //[Q2 type][package][run index num]
...
std::vector<std::vector<Double_t> > ANGLE_VAL;  //[package][run index num]

std::vector<std::vector<Double_t> > NOMINAL_RUN_Q2_VAL;  //[Q2 type][package] Q2 value for run NOMINAL_RUN what we are calling nominal

std::vector<std::vector<Double_t> > X_POSITION;  //[package][run index num]

//definition of integers
Int_t nentries;  // number of entries in the tree

const Int_t NUMPACKAGES = 3;
const Int_t NUMQ2TYPES = 2;

//"Nominal" run number, this is a global so that we can change it easily if
//needed without going through the code. Hard coding this would be very limiting,
//which is why I made it a global.
const Int_t NOMINAL_RUN = 15121;
...
std::string INDEXTOQ2[NUMQ2TYPES] = { "with Loss", "without Loss" };
...
//Values for the Beam position functions in the Angle and Q2

//Angle info from: http://dilbert.physics.wm.edu/Physics/148
Double_t ANGLE_SLOPE[NUMPACKAGES] = { 0, -0.019, 0.017 };  //deg/mm
Double_t ANGLE_INTERCEPT[NUMPACKAGES] = { 0, 7.748, 7.696 };  //deg

//Q2 info from: https://qweak.jlab.org/elog/Tracking/83
Double_t Q2_SLOPE[NUMPACKAGES] = { 0, -0.14, 0.12 };  //m(Gev)^2/mm
Double_t Q2_INTERCEPT[NUMPACKAGES] = { 0, 23.75, 23.4 };  //m(GeV)^2
Double_t NOMINAL_RUN_ANGLE_VAL[NUMPACKAGES] = { 0 };  //angle value for run NOMINAL_RUN what we are calling nominal
Double_t NOMINAL_RUN_X_POSITION[NUMPACKAGES] = { 0 };  //beam position for run NOMINAL_RUN

//the pass number
std::string PASS;

void Q2_vs_Run_BeamPositionCorrected(std::string pass, std::string filename)
{
  PASS = pass;
...
/***********************************************************
 ...
 Called By: Q2_vs_Run_BeamPositionCorrected
 Date: 05-17-2015
 Modified:
 *********************************************************/
void Loop_Through_Tree(TChain* track_that)
{
  ...
  ANGLE_VAL.resize(NUMPACKAGES);
  X_POSITION.resize(NUMPACKAGES);
  Q2_VAL.resize(NUMQ2TYPES);
  Q2_VAL_CORRECTED.resize(NUMQ2TYPES);
  Q2_ERROR.resize(NUMQ2TYPES);
  NOMINAL_RUN_Q2_VAL.resize(NUMQ2TYPES);
  ...
  for (int i_type = 0; i_type < NUMQ2TYPES; i_type++)
  {
    ...
    NOMINAL_RUN_Q2_VAL[i_type].resize(NUMPACKAGES);
    Q2_VAL_CORRECTED[i_type].resize(NUMPACKAGES);
    ...
    for (int i_pkg = 0; i_pkg < NUMPACKAGES; i_pkg++)
    {
      ...
      ANGLE_VAL[i_pkg].resize(RUNNUMLIST.size(), 0);
      X_POSITION[i_pkg].resize(RUNNUMLIST.size(), 0);
      Q2_VAL_CORRECTED[i_type][i_pkg].resize(RUNNUMLIST.size(), 0);
    }
  }
  ...
  for (int i_entry = 0; i_entry < nentries; i_entry++)
  {
      ...
      for (UInt_t i = 0; i < myQ2Loss->size(); i++)
      {
        ...
        switch ((*myQ2Loss)[i].R3package)
        {
          ...
          case 2:  //pkg 2
            ...
            Q2_VAL[0][(*myQ2Loss)[i].R3package][run_index] = 1000
                * (*myQ2Loss)[i].Val;
            Q2_ERROR[0][(*myQ2Loss)[i].R3package][run_index] = 1000
                * (*myQ2Loss)[i].Error;
      ...
      for (UInt_t i = 0; i < myQ2NoLoss->size(); i++)
      {
        ...
        switch ((*myQ2NoLoss)[i].R3package)
        {
          ...
          case 2:  //pkg 2
            ...
            //the Q2 values and error is multiplied by 1000 so it is in
            //m(Gev)^2 rather then GeV^2
            Q2_VAL[1][(*myQ2NoLoss)[i].R3package][run_index] = 1000
                * (*myQ2NoLoss)[i].Val;
            Q2_ERROR[1][(*myQ2NoLoss)[i].R3package][run_index] = 1000
                * (*myQ2NoLoss)[i].Error;
      ...
      for (UInt_t i = 0; i < myAngle->size(); i++)
      {
        ...
        switch ((*myAngle)[i].R3package)
        {
          ...
          case 2:  //pkg 2
            /*
             //debugging
             std::cout << "I am Scattering Angle hear me roar, this is run num: " << run
             << std::endl << std::endl;

             // for the Angle fill the vectors for the plots

             std::cout << "Type: " << INDEXTOQ2[3] << std::endl;
             std::cout << "Pkg: " << (*myAngle)[i].R3package << std::endl;
             std::cout << "Q2 val: " << (*myAngle)[i].Val << std::endl;
             */

            //the Angle values and error in Degrees
            ANGLE_VAL[(*myAngle)[i].R3package][run_index] = (*myAngle)[i].Val;
    ...
    /*************
     * Get the information for the Run we are considering "nominal"
    *************/
    if (run == NOMINAL_RUN)
    {
      for (UInt_t i = 0; i < myAngle->size(); i++)
      {
        /*
         //debugging
         std::cout << "In the run " << NOMINAL_RUN <<
         " Angle arena - we have data :-)" << std::cout;
         */
        ...
        switch ((*myAngle)[i].R3package)
        {
          ...
          case 2:  //pkg 2
            ...
            //the Angle value
            NOMINAL_RUN_ANGLE_VAL[(*myAngle)[i].R3package] = (*myAngle)[i].Val;

            NOMINAL_RUN_Q2_VAL[0][(*myQ2Loss)[i].R3package] = 1000
                * (*myQ2Loss)[i].Val;
            NOMINAL_RUN_Q2_VAL[1][(*myQ2NoLoss)[i].R3package] = 1000
                * (*myQ2NoLoss)[i].Val;
...
/***********************************************************
 ...
 Purpose:
 Get the beam position for run NOMINAL_RUN (the nominal position)

 Entry Conditions:
 none

 Global:
 - NOMINAL_RUN_ANGLE_VAL - values of the mean scattering angle for run NOMINAL_RUN
 - NOMINAL_RUN_X_POSITION - values of the beam position for run NOMINAL_RUN

 - ANGLE_SLOPE - the slope of the angle vs beam position
 - ANGLE_INTERCEPT -the intercept of the angle vs beam position
...
{
  /*****************
   * We have a slope and intercept for the scattering angle vs beam pos
   * Angle = Slope*position+Intercept
   *
   * we have chosen run NOMINAL_RUN for the nominal position, find find the position of the
   * beam in for this run.
   *
   * This is done from solving y = m*x +b solve for x leaving
   * x = (y - b)/m
   *****************/
  for (int i_pkg = 1; i_pkg < NUMPACKAGES; i_pkg++)
  {
    /*
     //debugging
     std::cout << "Made it in the Get Beam Position Function" << std::endl;
     */

    NOMINAL_RUN_X_POSITION[i_pkg] = (NOMINAL_RUN_ANGLE_VAL[i_pkg]
        - ANGLE_INTERCEPT[i_pkg])
                                 / ANGLE_SLOPE[i_pkg];

    /*
     //debugging
     std::cout << "The beam position of run " << NOMINAL_RUN <<
     " for pkg " << i_pkg << " is "
     << NOMINAL_RUN_X_POSITION[i_pkg] << " mm." << std::endl;
     */

    /****************
     * Find the beam position for the runs in the runlist
     *
     * found using the same formula
     ****************/
    for (int i_run = 0; i_run < RUNNUMLIST.size(); i_run++)
    {
      /*
       //debugging
       std::cout << "Made it in the loop to find the x position on all the runs in the runlist"
       << std::endl;
       */

      X_POSITION[i_pkg][i_run] = (ANGLE_VAL[i_pkg][i_run]
          - ANGLE_INTERCEPT[i_pkg])
                                 / ANGLE_SLOPE[i_pkg];

      /*
       //debugging
       std::cout << "The beam position of run " << i_run << " for pkg " << i_pkg
       << " is " << X_POSITION[i_pkg][i_run] << " mm." << std::endl;
       */
    }
  }
  return;
...
/***********************************************************
 Function: CorrectQ2
 Purpose:
 Correct beam position for each run in the run list.
...
{
  /*******************
   * we got the beam position from the GetBeamPosition function
   * Now we need to correct the Q2 value for each run back to the run that
   * is with the NOMINAL_RUN beam position
   *
   * This is done with with via the following eq:
   *    run NOMINAL_RUN = (x1, y1)
   *    run * = (x2, y2)
   *
   *    y = y2 + m(x1 - x2)
   *
  ...
  for (int i_type = 0; i_type < NUMQ2TYPES; i_type++)
  {
    for (int i_pkg = 1; i_pkg < NUMPACKAGES; i_pkg++)
    {
      for (int i_run = 0; i_run < RUNNUMLIST.size(); i_run++)
      {
        Q2_VAL_CORRECTED[i_type][i_pkg][i_run] = Q2_VAL[i_type][i_pkg][i_run]
            + (Q2_SLOPE[i_pkg] * (NOMINAL_RUN_X_POSITION[i_pkg] - X_POSITION[i_pkg][i_run]));

/*
        //debugging
        std::cout << "Run index number " << i_run << " corrected Q2 value "
        << Q2_VAL_CORRECTED[i_type][i_pkg][i_run] << std::endl;
*/
      }
    }
  }
  ...
  for (int i_pkg = 1; i_pkg < NUMPACKAGES; i_pkg++)
  {
    ...
    LEGEND[i_pkg] =
        (i_pkg == 0) ?
            ...
            new TLegend(0.80, 0.43, 0.99, 0.57);
    ...
    for (int i_type = 0; i_type < NUMQ2TYPES; i_type++)
    {
      ...
      g_Q2_VS_RUN[i_type][i_pkg] = new TGraphErrors(RUNNUMLIST.size(),
          &(RUN_VAL[0]), &(Q2_VAL_CORRECTED[i_type][i_pkg][0]), &(RUN_ERROR[0]),
      ...
      Q2_MAX[i_pkg] = std::max(Q2_MAX[i_pkg],
          *std::max_element(Q2_VAL_CORRECTED[i_type][i_pkg].begin(),
              Q2_VAL_CORRECTED[i_type][i_pkg].end()));

      Q2_MIN[i_pkg] = std::min(Q2_MIN[i_pkg],
          *std::min_element(Q2_VAL_CORRECTED[i_type][i_pkg].begin(),
              Q2_VAL_CORRECTED[i_type][i_pkg].end(), less_than_but_non_zero));
...
void Plot()
{
  ...
  for (int i_pkg = 1; i_pkg < NUMPACKAGES; i_pkg++)
  {
    ...
    c_q2[i_pkg]->SaveAs(
        Form(
            "~/qweak/QwAnalysis_trunk/Extensions/ValerianROOT/Pass%s_Analysis/Pass_%s_Q2_Corrected_vs_run_pkg_%s.png",
...
void PrintToFile()
{
  ...
  fout.open(
      Form(
          "/home/vmgray/qweak/QwAnalysis_trunk/Extensions/ValerianROOT/Pass%s_Analysis/Pass%s_Q2_Corrected_vs_Run.txt",
          PASS.c_str(), PASS.c_str()));

  if (!fout.is_open())
    cout << "File not opened" << endl;

  fout << "Run \t pkg \t\t Beam Pos \t\t 0 Val & Error "
  "\t\t\t 1 Val & Error \t\t\t 2 Val & Error \t\t\t 3 Val & Error"
  << std::endl;
  ...
  for (int i_run = 0; i_run < RUNNUMLIST.size(); i_run++)
  {
    for (int i_pkg = 1; i_pkg < NUMPACKAGES; i_pkg++)
    {
      ...
      << X_POSITION[i_pkg][i_run] << " \t\t "
      << Q2_VAL_CORRECTED[0][i_pkg][i_run] << " +- "
      << Q2_ERROR[0][i_pkg][i_run] << " \t\t\t "
      << Q2_VAL_CORRECTED[1][i_pkg][i_run] << " +- "
      << Q2_ERROR[1][i_pkg][i_run] << " \t\t\t " << Q2_VAL[0][i_pkg][i_run]
      << " +- " << Q2_ERROR[0][i_pkg][i_run] << " \t\t\t "
      << Q2_VAL[1][i_pkg][i_run] << " +- " << Q2_ERROR[1][i_pkg][i_run]
      << std::endl;
    }
...
//Table to Q2 type number to what it is
  ...
  for (int i_type = 0; i_type < NUMQ2TYPES; i_type++)
  {
    fout << i_type << " \t " << INDEXTOQ2[i_type].c_str() << " corrected"
    << std::endl;
  }

  for (int i_type = 0; i_type < NUMQ2TYPES; i_type++)
  {
    fout << i_type + 2 << " \t " << INDEXTOQ2[i_type].c_str() << std::endl;
  }
