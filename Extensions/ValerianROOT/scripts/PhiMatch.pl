...
#debug
print "The missing go, $MissingFile \n" if ($debug >= 2);
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $PhiMatchOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$PhiMatchOut \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( PHIMATCHOUT, ">$PhiMatchOut");
#      || die "Couldn't open file $PhiMatchOut, $!";
    if(-e $PhiMatchOut)
    {
      my $linecount = 0;

      while( my $line = <PHIMATCHDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        ...
        } else
          {
            ...
            for my $i (0 .. $#DataColumns)
            {
              if ($i != 0)
              {
                ...
                print PHIMATCHOUT "$DataColumns[$i] ";
              }
            }
      ...
      {
        warn "Couldn't open the file $PhiMatchOut $!";
      }
