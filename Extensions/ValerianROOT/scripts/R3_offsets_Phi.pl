...
#Purpose:
#
#This script takes the Phi offset  match up
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $PhiOffOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$PhiOffOut \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( PHIOFFOUT, ">$PhiOffOut");
#      || die "Couldn't open file $ThetaOffOut, $!";
    if(-e $PhiOffOut)
    {
      my $linecount = 0;

      while( my $line = <PHIOFFDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
      ...
      {
        warn "Couldn't open the file $PhiOffOut $!";
      }
