...
#Purpose:
#
#This script takes the Log Chi text files in for each run
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $ChiOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt"; 
  #debug
  print "$ChiOut \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator 
    ...
    open( CHIOUT, ">$ChiOut");
#      || die "Couldn't open file $ChiOut, $!";
    if(-e $ChiOut)
    {
      my $linecount = 0;

      while( my $line = <CHIDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        {
        ...
        } else
          {
            ...
            #for this (the Chi) file there are 6 entries in this
      ...
      {
        warn "Couldn't open the file $ChiOut $!";
      }
