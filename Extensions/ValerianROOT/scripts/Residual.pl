...
#Purpose:
#
#This script takes the Residual text files in for each run
#and rewrites them in a format that ROOT can input in
#with ease and care :)
#a happy root means a happy life
#
#Date: Star Wars day 2014 (May the 4th 2014)
#Modified: 06-05-2014
#Assisted By:
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $ResidualOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$ResidualOut \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( RESIDUALOUT, ">$ResidualOut");
#      || die "Couldn't open file $ResidualOut, $!";
    if(-e $ResidualOut)
    {
      my $linecount = 0;

      while( my $line = <RESIDUALDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        {
        ...
        } else
          {
            ...
            #we are going to take the data read in the RESIDUALDATA
            #file split it up into columns for each line (row)
            #and then print out the useful stuff
            my @DataColumns = split(' \t ', $line);

            #for this (the Residual) file there are 6 entries in this
            #array - I want to write out the last 4
            ...
            for my $i (0 .. $#DataColumns)
            {
              if($i != 0 && $i != 1)
              {
                ...
                #print RESIDUALOUT
                print RESIDUALOUT "$DataColumns[$i] ";
              }
            }
      ...
      {
        warn "Couldn't open the file $ResidualOut $!";
      }

    #close Residual data file
    close(RESIDUALDATA);
