...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $GoodTracksOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$GoodTracksOut \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( GOODTRACKSOUT, ">$GoodTracksOut");
#      || die "Couldn't open file $GoodTracksOut, $!";
    if(-e $GoodTracksOut)
    {
      my $linecount = 0;

      while( my $line = <GOODTRACKSDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        ...
        } else
          {
            ...
            for my $i (0 .. $#DataColumns)
            {
              if ($i != 0)
              {
                ...
                print GOODTRACKSOUT "$DataColumns[$i] ";
              }
            }
      ...
      {
        warn "Couldn't open the file $GoodTracksOut $!";
      }

    #close output data file
    close(GOODTRACKSDATA);
