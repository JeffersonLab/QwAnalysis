...
#Purpose:
#
#This script takes number of angel
...
#debug
print "The missing files go, $MissingFile \n" if ($debug >= 2);
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $AngleOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "We have and output file $AngleOut \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( ANGLEOUT, ">$AngleOut");
#      || die "Couldn't open file $AngleOut, $!";
    if(-e $AngleOut)
    {
      my $linecount = 0;

      while( my $line = <ANGLEDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
    ...
    }else
      {
        ...
        warn "Couldn't open the file $AngleOut $!";
      }
