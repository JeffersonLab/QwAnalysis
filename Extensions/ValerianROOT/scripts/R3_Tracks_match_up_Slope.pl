...
#Purpose:
#
#This script takes the slope of the tracks match up
...
#all other info 
my $script = "R3_Tracks_match_up_Slope";
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $TracksSlopeOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$TracksSlopeOut \n" if ($debug >= 2);
  ...
  open( TRACKSSLOPEDATA, "<$QweakText" );
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( TRACKSSLOPEOUT, ">$TracksSlopeOut");
#      || die "Couldn't open file $PTracksSlopeOut, $!";
    if(-e $TracksSlopeOut)
    {
      my $linecount = 0;

      while( my $line = <TRACKSSLOPEDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        {
        ...
        } else
          {
            ...
            for my $i (1 .. $#DataColumns)
            {
              ...
              print TRACKSSLOPEOUT "$DataColumns[$i] ";
            }
      ...
      close(TRACKSSLOPEOUT);
    }else
      {
        warn "Couldn't open the file $TracksSlopeOut $!";
      }

    #close PTracks slopes data file
    close(TRACKSSLOPEDATA);
