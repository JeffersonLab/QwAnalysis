...
#Purpose:
#
#This script takes the slope of the partial tracks match up
...
#all other info 
my $script = "R3_Partial_Tracks_match_up_Slope";
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $PTracksSlopeOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$PTracksSlopeOut \n" if ($debug >= 2);
  ...
  open( PTRACKSSLOPEDATA, "<$QweakText" );
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( PTRACKSSLOPEOUT, ">$PTracksSlopeOut");
#      || die "Couldn't open file $PTracksSlopeOut, $!";
    if(-e $PTracksSlopeOut)
    {
      my $linecount = 0;

      while( my $line = <PTRACKSSLOPEDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        {
        ...
        } else
          {
            ...
            #we are going to take the data read in the xxxDATA
            ...
            for my $i (1 .. $#DataColumns)
            {
              ...
              print PTRACKSSLOPEOUT "$DataColumns[$i] ";
            }
      ...
      close(PTRACKSSLOPEOUT);
    }else
      {
        warn "Couldn't open the file $PTracksSlopeOut $!";
      }

    #close PTracks slopes data file
    close(PTRACKSSLOPEDATA);
