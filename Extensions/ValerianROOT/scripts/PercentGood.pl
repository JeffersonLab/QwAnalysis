...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $PercentGoodOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$PercentGoodOut \n" if ($debug >= 2);
  ...
  open( PERCENTGOODDATA, "<$QweakText" );
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( PERCENTGOODOUT, ">$PercentGoodOut");
#      || die "Couldn't open file $PercentGoodOut, $!";
    if(-e $PercentGoodOut)
    {
      my $linecount = 0;

      while( my $line = <PERCENTGOODDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
      ...
      close(PERCENTGOODOUT);
    }else
      {
        warn "Couldn't open the file $PercentGoodOut $!";
      }

    #close output data file
    close(PERCENTGOODDATA);
