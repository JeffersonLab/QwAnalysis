...
#all other info 
my $script = "R3_Partial_Tracks_match_up_Chi2";
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $PTracksChiOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$PTracksChiOut \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( PTRACKSCHIOUT, ">$PTracksChiOut");
#      || die "Couldn't open file $PTracksChiOut, $!";
    if(-e $PTracksChiOut)
    {
      my $linecount = 0;

      while( my $line = <PTRACKSCHIDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        {
        ...
        } else
          {
            ...
            for my $i (1 .. $#DataColumns)
            {
               ...
               print PTRACKSCHIOUT "$DataColumns[$i] ";
            }
      ...
      {
        warn "Couldn't open the file $PTracksChiOut $!";
      }

    #close PTracks chi data file
    close(PTRACKSCHIDATA);
