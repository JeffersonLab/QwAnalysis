...
my $OutputDir = $YouAreHere . "/" . $script . "_R3_pkg1";
...
my $MissingFile = $MissingDir . "/" . $script . "R3Pkg1_Missing.txt" ;
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $OctantDeterminationOut = $OutputDir . "/" . $script . "_R3_pkg1_" . $runnum . ".txt"; 
  #debug
  ...
  my $QweakText = $QweakDir . "/run_" . $runnum . "/" . $script 
      . "_" . $runnum . "_" . $script . "_R3_pkg1.txt";
