...
#Purpose:
#
#This script takes the Chi2 of the tree line match up
...
#all other info 
my $script = "R3_Treeline_match_up_Chi2";
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $TreeChiOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$TreeChiOut \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( TREECHIOUT, ">$TreeChiOut");
#      || die "Couldn't open file $TreeChiOut, $!";
    if(-e $TreeChiOut)
    {
      my $linecount = 0;

      while( my $line = <TREECHIDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        {
        ...
        } else
          {
            ...
            for my $i (1 .. $#DataColumns)
            {
              ...
              #print TREECHIOUT
              print TREECHIOUT "$DataColumns[$i] ";
            }
      ...
      {
        warn "Couldn't open the file $TreeChiOut $!";
      }

    #close Tree chi data file
    close(TREECHIDATA);
