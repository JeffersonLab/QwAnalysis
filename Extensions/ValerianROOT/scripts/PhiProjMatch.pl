...
#Purpose:
#
#This script takes Phi Proj match
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $PhiProjMatchOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$PhiProjMatchOut \n" if ($debug >= 2);
  ...
  open( PHIPROJMATCHDATA, "<$QweakText" );
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( PHIPROJMATCHOUT, ">$PhiProjMatchOut");
#      || die "Couldn't open file $PhiProjMatchOut, $!";
    if(-e $PhiProjMatchOut)
    {
      my $linecount = 0;

      while( my $line = <PHIPROJMATCHDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        ...
        } else
          {
            ...
            for my $i (0 .. $#DataColumns)
            {
              if ($i != 0)
              {
                ...
                print PHIPROJMATCHOUT "$DataColumns[$i] ";
              }
            }
      ...
      close(PHIPROJMATCHOUT);
    }else
      {
        warn "Couldn't open the file $PhiProjMatchOut $!";
      }

    #close output data file
    close(PHIPROJMATCHDATA);
