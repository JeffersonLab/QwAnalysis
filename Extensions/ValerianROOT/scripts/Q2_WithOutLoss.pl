...
#Purpose:
#
#This script takes number of Q2 with out loss
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $Q2Out = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$Q2Out \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( Q2OUT, ">$Q2Out");
#      || die "Couldn't open file $Q2Out, $!";
    if(-e $Q2Out)
    {
      my $linecount = 0;

      while( my $line = <Q2DATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
      ...
      {
        warn "Couldn't open the file $Q2Out $!";
      }
