...
#Purpose:
#
#This script takes scattering energy
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $ScatteringEnergyOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$ScatteringEnergyOut \n" if ($debug >= 2);
  ...
  open( SCATTERINGENERGYDATA, "<$QweakText" );
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( SCATTERINGENERGYOUT, ">$ScatteringEnergyOut");
#      || die "Couldn't open file $ScatteringEnergyOut, $!";
    if(-e $ScatteringEnergyOut)
    {
      my $linecount = 0;

      while( my $line = <SCATTERINGENERGYDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        ...
        } else
          {
            ...
            for my $i (0 .. $#DataColumns)
            {
              if ($i != 0)
              {
                ...
                print SCATTERINGENERGYOUT "$DataColumns[$i] ";
              }
            }
      ...
      close(SCATTERINGENERGYOUT);
    }else
      {
        warn "Couldn't open the file $ScatteringEnergyOut $!";
      }

    #close output data file
    close(SCATTERINGENERGYDATA);
