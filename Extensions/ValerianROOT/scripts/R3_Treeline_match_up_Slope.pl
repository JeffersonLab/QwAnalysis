...
#Purpose:
#
#This script takes the slope of the tree line match up
...
#all other info 
my $script = "R3_Treeline_match_up_Slope";
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $TreeSlopeOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$TreeSlopeOut \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( TREESLOPEOUT, ">$TreeSlopeOut");
#      || die "Couldn't open file $TreeSlopeOut, $!";
    if(-e $TreeSlopeOut)
    {
      my $linecount = 0;

      while( my $line = <TREESLOPEDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        {
        ...
        } else
          {
            ...
            for my $i (1 .. $#DataColumns)
            {
              ...
              #print TREESLOPEOUT
              print TREESLOPEOUT "$DataColumns[$i] ";
            }
      ...
      {
        warn "Couldn't open the file $TreeSlopeOut $!";
      }

    #close Tree slopes data file
    close(TREESLOPEDATA);
