...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $ThetaMatchOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$ThetaMatchOut \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( THETAMATCHOUT, ">$ThetaMatchOut");
#      || die "Couldn't open file $ThetaMatchOut, $!";
    if(-e $ThetaMatchOut)
    {
      my $linecount = 0;

      while( my $line = <THETAMATCHDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        {
        ...
        } else
          {
            ...
            #we are going to take the data read in the DATA
            ...
            for my $i (0 .. $#DataColumns)
            {
              if ($i != 0)
              {
                ...
                print THETAMATCHOUT "$DataColumns[$i] ";
              }
            }
      ...
      {
        warn "Couldn't open the file $ThetaMatchOut $!";
      }

    #close output data file
    close(THETAMATCHDATA);
