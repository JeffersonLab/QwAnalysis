...
#Purpose:
#
#This script takes number of raw tracks
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $RawTracksOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$RawTracksOut \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( RAWTRACKSOUT, ">$RawTracksOut");
#      || die "Couldn't open file $RawTracksOut, $!";
    if(-e $RawTracksOut)
    {
      my $linecount = 0;

      while( my $line = <RAWTRACKSDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        ...
        } else
          {
            ...
            for my $i (0 .. $#DataColumns)
            {
              if ($i != 0)
              {
                ...
                print RAWTRACKSOUT "$DataColumns[$i] ";
              }
            }
      ...
      {
        warn "Couldn't open the file $RawTracksOut $!";
      }
