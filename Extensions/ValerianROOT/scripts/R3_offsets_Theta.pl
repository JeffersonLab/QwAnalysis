...
#Purpose:
#
#This script takes the Theta offset  match up
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $ThetaOffOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$ThetaOffOut \n" if ($debug >= 2);
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( THETAOFFOUT, ">$ThetaOffOut" );
#      || die "Couldn't open file $ThetaOffOut, $!";
    if(-e $ThetaOffOut)
    {
      my $linecount = 0;

      while( my $line = <THETAOFFDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        ...
        } else
          {
            ...
            for my $i (0 .. $#DataColumns)
            {
              if ($i != 0 && $i != 2)
              {
                ...
                print THETAOFFOUT "$DataColumns[$i] ";
              }
            }
      ...
      {
        warn "Couldn't open the file $ThetaOffOut $!";
      }
