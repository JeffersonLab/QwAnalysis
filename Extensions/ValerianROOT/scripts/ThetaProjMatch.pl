...
#Purpose:
#
#This script takes theta Proj match
...
while ( my $runnum = <ALLRUNS> )
{
  ...
  my $ThetaProjMatchOut = $OutputDir . "/" . $script . "_" . $runnum . ".txt";
  #debug
  print "$ThetaProjMatchOut \n" if ($debug >= 2);
  ...
  open( THETAPROJMATCHDATA, "<$QweakText" );
...
#die hard aborts the script, we just want it to skip the
  ...
  { # -e is the exist operator
    ...
    open( THETAPROJMATCHOUT, ">$ThetaProjMatchOut");
#      || die "Couldn't open file $ThetaProjMatchOut, $!";
    if(-e $ThetaProjMatchOut)
    {
      my $linecount = 0;

      while( my $line = <THETAPROJMATCHDATA> )
      {
        ++$linecount;
        chomp( $line );
        if ($linecount == 1)
        ...
        } else
          {
            ...
            for my $i (0 .. $#DataColumns)
            {
              if ($i != 0)
              {
                ...
                print THETAPROJMATCHOUT "$DataColumns[$i] ";
              }
            }
      ...
      close(THETAPROJMATCHOUT);
    }else
      {
        warn "Couldn't open the file $ThetaProjMatchOut $!";
      }

    #close output data file
    close(THETAPROJMATCHDATA);
