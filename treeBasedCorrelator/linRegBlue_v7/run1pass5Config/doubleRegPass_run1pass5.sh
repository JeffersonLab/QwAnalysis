...
#Al range with Uslumi issues
	...
	#  Don't do anything, because the confDir was set by
...
# run1pass5_transverse_al carry the same 15 uA current cut as run1pass5_bcm-cut-15
if [ $run -ge 9824  -a  $run -le 9844 ] ; then
    confDir="run1pass5_transverse_lh2"
elif [ $run -ge 9845  -a  $run -le 9858 ] ; then
    confDir="run1pass5_transverse_al"
...
if [ -d ${workPath} ] ; then
    echo "Using LRB config file: ${scriptPath}/$confDir/${confFile}"
