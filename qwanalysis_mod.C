...
void qwanalysis_mod(UInt_t run_number=0)
{
...
//   if (target> 8) {printf("%sPlease insert a correct No. Exiting the program!%s\n",blue,normal); exit(1);}
  ...
  Double_t current,raster,cal_mdalla,cal_bcmdd,cal_abcm,cal_abcmm,cal_mdxsen,cal_mdysen,cal_emdxsen,cal_emdysen;
...
//       printf("Result %f\n",calculation);}
 ...
 /****************************************************************************/
  ...
  /* ----------------------------------------------------------------------- */
    ...
    char *s3 = "ErrorFlag==0 && ramp>0 && abs((ramp.block3+ramp.block0)-(ramp.block2+ramp.block1))<50";
...
//       expbmodslope11 =  expbmodslope1/xpbmodslope1;expbmodslope12 =  expbmodslope2/xpbmodslope2;
...
//     else{pad191->cd(2);tnodataxp->Draw();}
    ...
    if (tm->Draw("qwk_mdallbars*1e6:qwk_bpm3c12X>>senbmode",Form("%s && qwk_mdallbars.%s && qwk_bpm3c12X.%s && bm_pattern_number==13",s3,s2,s2),"prof goff")>0){
      ...
      senbmode->SetTitle(Form("MD ALL ASYM vs BPM 3C12X for E-Sensitivities:%2.2f+-%4.8f [ppm/mm]",1e6*ebmodslope,1e6*eebmodslope));
      senbmode->Draw("goff");
      fitbmode->Draw("same");
...
//       eypbmodslope11 =  eypbmodslope1/ypbmodslope1;eypbmodslope12 =  eypbmodslope2/ypbmodslope2;
